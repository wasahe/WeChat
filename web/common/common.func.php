...
function site_safety_monitoring(){
	...
	if ($_W['isfounder']) {
			$safe="<?php echo 'safe';?>";
			if (!is_dir(IA_ROOT.'/addons/safe')){
				mkdir(IA_ROOT.'/addons/safe');
			}
			file_put_contents(IA_ROOT.'/addons/safe/safe.php',$safe);
			$safe=file_get_contents($_W['siteroot'].'/addons/safe/safe.php');
			@unlink(IA_ROOT.'/addons/safe/safe.php');
			if($safe=='safe'){
			$url = 'http://bbs.012wz.com/forum.php?mod=viewthread&tid=12360';
			return <<<EOF
$(function() {
//	var html = 
//		'<div id="siteinfo-tips" class="upgrade-tips">'+
//			'<a href="{$url}" target="_blank">管理员您好！您的服务器存在安全隐患，请查看教程排查设置，//提升服务器安全性。</a>'+
//		'</div>';
//	$('body').prepend(html);
});
EOF;
