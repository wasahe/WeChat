<?php
	...
	$openid = $_W['openid'];
	$arr = pdo_fetch("SELECT * FROM " . tablename('deam_puzzleredpack_packetset') . " WHERE id = :id AND uniacid = :uniacid", array(':id' => $act_id,':uniacid' => $uniacid));
	if(empty($arr)){
		exit('活动不存在！');
        die();
	}
	$isprize = pdo_fetch("SELECT * FROM " . tablename('deam_puzzleredpack_record') . " WHERE actid = :actid AND uniacid = :uniacid AND openid = :openid ORDER BY `id` DESC", array(':actid' => $act_id,':uniacid'=>$uniacid,':openid'=>$openid));
	if($isprize){
	...
	if(!$_COOKIE['deam_openid']){
		if($_GPC['openid']){
			...
			header("Location: ".$this->createMobileUrl('puzzle',array('id'=>$act_id)));
			exit;
		}
