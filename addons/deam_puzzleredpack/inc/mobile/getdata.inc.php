...
$actInfo = pdo_fetch("SELECT * FROM " . tablename('deam_puzzleredpack_packetset') . " WHERE id = :id AND uniacid = :uniacid", array(':id' => $actid,':uniacid'=>$uniacid));
...
if($is_subscribe && !$isGuanzhu){//开启了必须关注且没有关注
	$date['status']=40007;
	$date['msg'] = '未关注';
	echo json_encode($date);
	exit();
...
$isprize = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('deam_puzzleredpack_record') . " WHERE actid = :actid AND uniacid = :uniacid AND openid = :openid", array(':actid' => $actid,':uniacid'=>$uniacid,':openid'=>$openid));
if($isprize){
	$date['status']=40004;
	$date['msg'] = '已中奖';
	echo json_encode($date);
...
if($nowtime < $starttime){
	...
	$alreadyPost = pdo_fetchcolumn("SELECT SUM(total_amount) FROM " . tablename('deam_puzzleredpack_record') . " WHERE actid =".$actid." AND uniacid = ".$uniacid);//已经领的金额
	$price_num = mt_rand($actInfo['minprize']*100,$actInfo['maxprize']*100);//随机金额，单位：分
	...
	if($is_packet == '1'){
		...
		if($insertid){
			...
			$hborderinfo = simplexml_load_string($this->curl_post_ssl("https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack",Deam_puzzleredpackModuleSite::arrayToXml($postdate)),'SimpleXMLElement', LIBXML_NOCDATA);//红包预下单返回信息
			...
			if($hborderinfo['return_code'] == 'SUCCESS'){
				...
				pdo_update('deam_puzzleredpack_record',$packetUpdate,array('id' => $insertid));
				$getmoney = number_format($price_num / 100 , 2);
				$date['status'] = 101;
				$date['money'] = $getmoney;
			}else{
				pdo_delete('deam_puzzleredpack_record', array('id' => $insertid));
