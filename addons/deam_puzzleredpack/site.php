...
class Deam_puzzleredpackModuleSite extends WeModuleSite {
	...
	public function doWebActset() {
		...
		if($operation == 'post'){
			$id = intval($_GPC['id']);
			if (!empty($id)) {
				$item = pdo_fetch("SELECT * FROM " . tablename('deam_puzzleredpack_packetset') . " WHERE id = :id AND uniacid = :uniacid", array(':id' => $id,':uniacid'=>$uniacid));
				if (empty($item)) {
			...
			if(checksubmit('submit')){
				...
				if (empty($id)) {
					...
					$result = pdo_update("deam_puzzleredpack_packetset", $data,array("id"=>$id));
				}
				if(!empty($result)){
	...
	public function doWebPacketRecord(){
		...
		$item = pdo_fetch("SELECT * FROM " . tablename('deam_puzzleredpack_packetset') . " WHERE id = :id AND uniacid = :uniacid", array(':id' => $actid,':uniacid'=>$uniacid));
		
		$firstpacket = pdo_fetch("SELECT * FROM " . tablename('deam_puzzleredpack_record') . " WHERE actid = :actid AND uniacid = :uniacid ORDER BY `id` ASC", array(':actid' => $actid,':uniacid'=>$uniacid));
		$lastpacket = pdo_fetch("SELECT * FROM " . tablename('deam_puzzleredpack_record') . " WHERE actid = :actid AND uniacid = :uniacid ORDER BY `id` DESC", array(':actid' => $actid,':uniacid'=>$uniacid));
		...
		//$packetState = $packetStateinfo['hblist']['hbinfo']['status'];
	...
	public function doWebPacketRecordDel(){
		...
		$result = pdo_delete('deam_puzzleredpack_record', array('actid' => $actid , 'id' => $packetid , 'uniacid' => $uniacid ));
		if (!empty($result)) {
			message('删除成功', $this->createWebUrl('packetrecord', array('id' => $actid)), 'success');
		}
	}
	...
	public function getPacketStatus($mch_billno){
		...
		$packetStateinfo = simplexml_load_string($this->curl_post_ssl($packetStateUrl,Deam_puzzleredpackModuleSite::arrayToXml($postDate)),'SimpleXMLElement', LIBXML_NOCDATA);//红包预下单返回信息
	...
	public function checkIsSubscribe($subscribe_model){
		...
		if($subscribe_model == '2'){
			...
			if($_COOKIE['deam_openid']){
				...
				$isGuanzhu =  pdo_fetch("SELECT * FROM " . tablename('deam_puzzleredpack_guanzhu') . " WHERE openid = :openid AND uniacid = :uniacid", array(':openid' => $_COOKIE['deam_openid'],':uniacid'=>$uniacid));
				if(!empty($isGuanzhu)){
					$isSubscribe = '1';
				}else{
					$isSubscribe = '0';
