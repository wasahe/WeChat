...
//require_once IA_ROOT .'/addons/tiger_jifenbao/weixinutil.class.php';
class Tiger_jifenbaoModuleReceiver extends WeModuleReceiver {
	public function receive() {
		...
		if($this->message['event']=="subscribe"){
            ...
            $status1=1;
            pdo_update('tiger_jifenbao_share',array('follow'=>$status1),array('openid'=>$mc['uid'],'weid'=>$mc['uniacid'])); 
            ...
            $zfs=$cfg["tiger_jifenbao_fansnum"]+$fanssum;
            $url=$_W['siteroot'].str_replace('./','app/',$this->createMobileurl('kending',array('uid'=>$mc['uid'])));
			$msg=htmlspecialchars_decode($cfg["tiger_jifenbao_usr"]);
		...
		if ($this->message['msgtype'] == 'event') {
			...
			if ($this->message['event'] == 'unsubscribe') {
                $mc = mc_fetch($this->message['from']);
                $reply=pdo_fetch('select * from '.tablename('tiger_jifenbao_poster').' where weid=:weid order by id asc limit 1',array(':weid'=>$mc['uniacid']));
                if($reply['rscore']==0){
                  return '';
                }
                ...
                $m_fans=pdo_fetch('select * from '.tablename('tiger_jifenbao_share').' where weid=:weid AND from_user=:from_user order by id asc limit 1',array(':weid'=>$mc['uniacid'],':from_user'=>$this->message['from']));
               // if($m_fans['helpid']){
                if ($m_fans['helpid'] && empty($m_fans['hasdel'])){
                  $s_fans=pdo_fetch('select * from '.tablename('tiger_jifenbao_share').' where weid=:weid AND openid=:openid order by id asc limit 1',array(':weid'=>$mc['uniacid'],':openid'=>$m_fans['helpid']));
                 ...
                 // }
                  ...
                  if($s_fans['helpid']){
                    $p_fans=pdo_fetch('select * from '.tablename('tiger_jifenbao_share').' where weid=:weid AND openid=:openid order by id asc limit 1',array(':weid'=>$mc['uniacid'],':openid'=>$s_fans['helpid']));
                    //if($p_fans['follow']==0){
                    //  exit;
                    //}
                ...
                $status=0;
                pdo_update('tiger_jifenbao_share',array('follow'=>$status,'hasdel'=>1),array('openid'=>$mc['uid'],'weid'=>$mc['uniacid'])); 
