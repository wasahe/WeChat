...
class Tiger_jifenbaoModuleProcessor extends WeModuleProcessor
{
	public function respond()
	{
		...
		if ($this->message['content'] == '肯定好友') {
			...
			if ($_W['account']['level'] == 4) {
				...
				if (empty($credit1) || empty($credit1)) {
					...
					$hmember = pdo_fetch("SELECT * FROM " . tablename('tiger_jifenbao_share') . " WHERE weid = :weid and openid=:openid", array(':weid' => $_W['uniacid'], ':openid' => $share['helpid']));
		...
		if ($this->message['msgtype'] == 'event') {
			...
			if ($this->message['event'] == 'subscribe') {
				...
				$member = pdo_fetch("SELECT * FROM " . tablename('tiger_jifenbao_share') . " WHERE weid = :weid and from_user=:from_user", array(':weid' => $_W['uniacid'], ':from_user' => $this->message['from']));
				$cfg = $this->module['config'];
				if (empty($member)) {
		...
		if ($this->message['msgtype'] == 'text' || $this->message['event'] == 'CLICK') {
			...
			$rid = $this->rule;
			$poster = pdo_fetch("SELECT * FROM " . tablename('tiger_jifenbao_poster') . " WHERE weid = :weid and rid=:rid", array(':weid' => $_W['uniacid'], ':rid' => $rid));
	...
	private $sceneid = 0;
	private $Qrcode = "/addons/tiger_jifenbao/qrcode/mposter#sid#.jpg";
	private function createPoster($fans, $poster)
	{
		...
		if (empty($share)) {
			pdo_insert($this->modulename . "_share", array('openid' => $fans['uid'], 'nickname' => $fans['nickname'], 'avatar' => $fans['avatar'], 'pid' => $poster['id'], 'updatetime' => time(), 'createtime' => time(), 'parentid' => 0, 'weid' => $_W['uniacid'], 'score' => $poster['score'], 'cscore' => $poster['cscore'], 'pscore' => $poster['pscore'], 'from_user' => $this->message['from'], 'follow' => 1));
			$share['id'] = pdo_insertid();
			$share = pdo_fetch('select * from ' . tablename($this->modulename . "_share") . " where id='{$share['id']}'");
		} else {
			pdo_update($this->modulename . "_share", array('updatetime' => time()), array('id' => $share['id']));
		}
		...
		foreach ($data as $value) {
			...
			if ($value['type'] == 'qr') {
				...
				if (!empty($url)) {
					$img = IA_ROOT . "/addons/tiger_jifenbao/temp_qrcode.png";
	...
	private function getQR($fans, $poster, $sid)
	{
		...
		if (empty($this->sceneid)) {
			...
			pdo_insert('qrcode', array('uniacid' => $_W['uniacid'], 'acid' => $acid, 'qrcid' => $this->sceneid, 'name' => $poster['title'], 'keyword' => $row['content'], 'model' => 1, 'ticket' => $res['ticket'], 'expire' => $time, 'createtime' => time(), 'status' => 1, 'url' => $res['url']));
