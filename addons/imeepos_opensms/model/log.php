...
class ImeeposOpenSms_log
{
    ...
    public function install(){
        ...
        if(!pdo_fieldexists($this->table,'nickname')){
            pdo_query("ALTER TABLE ".tablename($this->table)." ADD COLUMN `nickname` varchar(32) DEFAULT ''");
        }
        ...
        if(!pdo_fieldexists($this->table,'type')){
            pdo_query("ALTER TABLE ".tablename($this->table)." ADD COLUMN `type` tinyint(3) DEFAULT '0'");
        }
    }
}