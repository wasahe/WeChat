...
class weisrc_friendModuleSite extends WeModuleSite
{
    ...
    public function doMobileindex()
    {
        ...
        if (empty($tofrom_user)) {
            $method = 'index';
            $authurl = $_W['siteroot'] . 'app/' . $this->createMobileUrl($method, array('id' => $id, 'sex' => $sex), true) . '&authkey=1';
            $url = $_W['siteroot'] . 'app/' . $this->createMobileUrl($method, array('id' => $id, 'sex' => $sex), true);
            if (isset($_COOKIE[$this->_auth2_openid])) {
...
//            }
            ...
            if (empty($fans)) {
                ...
                $data = array('nickname' => $nickname, 'headimgurl' => $headimgurl, 'sex' => $sex);
                if (!isset($_GPC['sex'])) {
                    unset($data['sex']);
                }
        ...
        if ($sub == 0) {
            if (empty($follow_url)) {
                $follow_url = $this->createMobileUrl('index', array('id' => $id, 'sex' => 1), true);
                $follow_url2 = $this->createMobileUrl('index', array('id' => $id, 'sex' => 2), true);
            }
        } else {
            $follow_url = $this->createMobileUrl('index', array('id' => $id, 'sex' => 1), true);
            $follow_url2 = $this->createMobileUrl('index', array('id' => $id, 'sex' => 2), true);
