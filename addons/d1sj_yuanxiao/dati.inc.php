...
// 		$order_res=pdo_fetchall('select count(ti_id) as dati_shu from '.tablename('healer_yuanxiao_correct')." where eid=:eid and openid=:openid and order_no=:order_no ",array(':eid'=>$_W['uniacid'],':openid'=>$openid,':order_no'=>$order_no));
...
// 	$only=$_GPC['only'];
// 	//答题正确插入正确数据
// 	$correct_res=pdo_fetch('select * from '.tablename('healer_yuanxiao_dm').' where id= '.$ti_id);

// 	$res=pdo_fetch('select * from '.tablename('healer_yuanxiao_correct').' where ti_id= '.$ti_id.' and order_no ='.$order_no);
...
// 				'only'    =>$only,
// 				);
// 			$rs=pdo_insert("healer_yuanxiao_correct",$data);
		
// 	}

// 	$ti_list=pdo_fetchall('select * from '.tablename('healer_yuanxiao_dm').' where uniacid=:uniacid and id >'.$ti_id." limit 1 ",array(':uniacid'=>$_W['uniacid']));
// 	$timu_list=array();
// 	if($ti_list){
...
// 		$order_res=pdo_fetchall('select count(ti_id) as dati_shu from '.tablename('healer_yuanxiao_correct')." where eid=:eid and openid=:openid and order_no=:order_no ",array(':eid'=>$_W['uniacid'],':openid'=>$_W['openid'],':order_no'=>$order_no));
...
// }else{
	$order_no=order();
	$ti_list=pdo_fetchall('select * from '.tablename('healer_yuanxiao_dm').' where uniacid=:uniacid limit 1',array(':uniacid'=>$_W['uniacid']));
	$timu_list=array();
