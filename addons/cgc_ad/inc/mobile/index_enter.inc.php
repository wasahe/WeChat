...
if($_GPC['dopost']=='ajax'){
	...
	$list=pdo_fetchall("SELECT a.*,b.type,b.thumb,b.nicheng,b.nickname,b.headimgurl,b.openid FROM ".tablename('cgc_ad_adv')." as a
			...
			WHERE a.weid=".$weid."  AND a.quan_id=".$quan_id." AND del=0 AND a.status=1 ORDER BY rob_end_time asc,a.top_level DESC,a.create_time desc,a.id DESC limit  ".$__pages.",10");
	...
	foreach ($list as $key => $item) {
		...
		.'<span class="sdavatar">';
		if($item['type']==1){
			$ht.='<img src="'.$item['headimgurl'].'"/>';
		}else{
			$ht.='<img src="'.tomedia($item['thumb']).'"/>';
		}
		$ht.='</span>
		...
		if($item['rob_end_time']>0){
			...
			$ht.='<span class="sdmoney over">'.$item['total_amount'].'元</span>';
			if($item['top_level']>0){
			...
			$ht.='<span class="sdmoney">'.$item['total_amount'].'元 ';
...
$total = pdo_fetch("SELECT SUM(total_amount)  total_amount,SUM(views)  views FROM ".tablename('cgc_ad_adv')." where weid=".$weid." AND quan_id=".$quan_id);

//总撒钱
$_stotal=$total['total_amount'];

//总人气
$_pvtotal=$total['views'];
