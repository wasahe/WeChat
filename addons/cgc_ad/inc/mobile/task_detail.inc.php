<?php
	...
	if($op=='display'){
	  ...
	  if (!empty($adv['job_submission_time']) && $quan['task_submit_switch']){
	    $clear_task=pdo_query("DELETE FROM ".tablename('cgc_ad_task')." WHERE weid=".$weid." AND quan_id=".$adv['quan_id']." AND advid=".$id."  AND task_status=0  and status=0 AND create_time<".(time()-60*$adv['job_submission_time']));  	 
	  }
   ...
   if($op=='check'){   
        ...
        if ($ret['code']=="1"){
          ...
          if (!empty($config['task_template_id']) && $member['message_notify']){      	  
   	  	    $_tdata = array(
	                       ...
	                       'keyword2'=>array('value'=>"你的任务已经审核通过，获得了".$ret['get_money'].$config['unit_text'].",点此到个人中心查看",'color'=>'#173177'),	              
      ...
      if ($total){
       ...
       $this->returnError("审核失败，失败原因:".$err_msg);
      }
        ...
        if ($ret){
          $total=$total+1;
            ...
            " WHERE   weid=".$weid." AND quan_id=".$quan_id." and id='{$task['mid']}'");          
       	  if (!empty($config['task_template_id']) && $notify_member['message_notify']){      	  
   	  	    $_tdata = array(
	                       'first'=>array('value'=>'任务反馈信息！','color'=>'#173177'),
	                       'keyword1'=>array('value'=>"任务模式红包",'color'=>'#173177'),
   ...
   if($op=='del_select'){   
	  ...
	  $ret=pdo_query("delete FROM ".tablename('cgc_ad_task')." WHERE weid=".$weid." AND quan_id=".$quan_id." AND advid=".$id." AND id in (".$_GPC['task_id'].") and status=0");
    ...
    foreach ($red as $key => $r) {   	
    	  if ($check_auth){
    	    if ($r['status']==1){
    	     ...
    	     $checkbox='<input class="weui_cell_sd task_status'.$r['task_status'].'" type="checkbox" name="checkbox[]"  value="'.$r['id'].'"/>';
            }
    	  } 
    ...
    if($quan['task_guanzhu']==1){
     ...
     $task=pdo_fetch("select * from " . tablename('cgc_ad_task') . " where weid=:uniacid and quan_id=:quan_id and advid=:advid and mid=:mid", array(':uniacid' => $_W['uniacid'],':quan_id' => $quan_id,':advid' => $id,':mid' =>$mid));  	
   	 ...
   	 $entity=array(
   	  ...
   	  if (empty($ret)){
   	    ...
   	    if (!empty($config['task_template_id'])){
            ...
            " WHERE   weid=".$weid." AND quan_id=".$quan_id." and openid='{$adv['openid']}'");
   	    	if (($adv_member['message_notify']) && empty($quan['task_submit_switch'])){
   	  	      $_tdata = array(
	                       'first'=>array('value'=>'任务反馈信息！','color'=>'#173177'),
	                       'keyword1'=>array('value'=>"任务模式红包",'color'=>'#173177'),
   	  ...
   	  $msg="领取任务成功。";
   	  if ($quan['task_submit_switch'] && $adv['job_submission_time']){
   	    $msg.="请你于".$adv['job_submission_time']."分钟之内在本页面提交任务。";
   	  } else 
   	  if ($quan['task_submit_switch']){
   	    $msg.="完成后请你在本页面提交任务。";
   	  }
