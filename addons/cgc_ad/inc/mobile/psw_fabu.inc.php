<?php
 ...
 if($op == 'display'){
     ...
     include $this -> template('psw_fabu');
     exit();
     }
...

{
    ...
    if(empty($total_amount) || $total_amount <= 0){
     ...
     if($total_amount < $quan['total_min7']){
        
         $this -> returnError('广告金额不能低于' . $quan['total_min7'] . $config['uni_text']);
        
         }
    
     if($total_amount > $quan['total_max7']){
        
         $this -> returnError('广告金额不能超过' . $quan['total_max7'] . $config['uni_text']);
    ...
    if(empty($total_num) || $total_num < 1){
     ...
     if($total_num > intval($total_amount / $quan['avg_min7'])){
        
         $this -> returnError($total_amount . $config['uni_text'].'最多可分' . intval($total_amount / $quan['avg_min7']) . '份');
        
         }
    
    
    
     $least_amount = max($quan['total_min7'], $total_num * $quan['avg_min7']);
