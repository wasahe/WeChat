...
if ($op=="display"){	 
   ...
   // 获取我的团队
    ...
    if(!empty($inviter) && $inviter['id']!=$member['id']){	  	
	  if ($quan['group_guanzhu'] && empty($subscribe)){	
	  	if (empty($quan['follow_url'])) {
	  	    $this -> returnError('组团强制关注，却没有设置关注地址url' );
	  	}
  ...
  // 生成我的专属分享链接
   $share_url=$_W['siteroot'] . 'app/' . substr($this->createMobileUrl('foo',array('form'=>'group_detail','op'=>'group_detail','quan_id'=>$quan_id,'src'=>$member['id'],'pid'=>$member['id'],'id'=>$adv['id'])), 2);
  ...
  $red=pdo_fetchall("SELECT a.mid,a.money,b.type,b.nicheng,b.thumb,b.headimgurl,b.nickname FROM ".tablename('cgc_ad_red')." as a
  left join ".tablename('cgc_ad_member')." as b on a.mid=b.id
  WHERE a.weid=".$weid." AND a.quan_id=".$adv['quan_id']." AND a.advid=".$id." ORDER BY a.create_time DESC limit $__pages,$pagesize",array(),"mid");
  if (!empty($red)){
