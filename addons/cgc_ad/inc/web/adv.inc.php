				...
				if ($op == 'jiqi') {
					...
					if ($op == 'post') {
						...
						if (checksubmit()) {
                           ...
                           // $_url = $_W['siteroot'] ."app/".substr($this->createMobileUrl('foo',array('form'=>'index','quan_id'=>$fabu_xinxi['quan_id'])), 2);
...
elseif ($op == 'del') {
	...
	if ($op == 'sendmsg') {
		...
		//$_url = $_W['siteroot'] ."app/".substr($this->createMobileUrl('foo',array('form'=>'index','quan_id'=>$temp['quan_id'])), 2);
