...
class Amouse_RebateModule extends WeModule
{
    public function settingsDisplay($settings)
    {
       ...
       // load()->func('file');
      //  $uniacid = $_W['uniacid'];
      //  $mod = pdo_fetch("SELECT * FROM " . tablename('modules') . "where name='amouse_rebate'");
      //  $packet = cloud_m_build($mod['name']);
       // if ($packet['manifest']) {
      //      $manifest = ext_module_manifest_parse($packet['manifest']);
       // } else {
       //    $manifest = ext_module_manifest($mod['name']);
       // }
       // if (is_array($manifest) && ver_compare($mod['version'], $manifest['application']['version']) == '-1') {
       //     $mod['upgrade'] = true;
       // }
       // include $this->template('web/sysset_upgrade');
	   message('悟空源码网(www.5kym.com)开发中，暂时用不到。') ;
    }
}
...
function ext_module_manifest_parse($xml)
{
    ...
    if (!empty($platform)) {
        $manifest['platform'] = array('subscribes' => array(), 'handles' => array(), 'isrulefields' => false, 'iscard' => false);
    ...
    if (!empty($permissions)) {
        ...
        for ($i = 0; $i < $items->length; $i++) {
            $item = $items->item($i);
            $row = array('title' => $item->getAttribute('title'), 'permission' => $item->getAttribute('do'));
