...
//	
	// 选择规格
	$("p.gattr").click(function(){
		$(this).siblings().removeClass('select');
		$('input[name=attr]').val($(this).text());
		$(this).addClass('select');
	});


	// 删除购物车商品
	$(".clear_t_goods").click(function(){
		if(!confirm('确定要删除该商品吗?')){
			return false;
		}
		var that = $(this);
		var key = $(this).attr('cartkey');
		$.get(clear_cartgoods,{'key':key},function(data){

			var info = eval(data);
			if(info.s == 'ok'){
				that.parent('li').css('display','none');
				$(".cart_total").text(info.total+'元');
	...
	// 购车商品数量增加
	$("a.cart_addg").click(function(){
		var key = $(this).parent('.changnum').attr('cartkey');
		var that = $(this); 
		$.get(change_cartgoods,{'addtype':'add','key':key},function(data){
			var info = eval(data);
			that.siblings('.t_cnum').text(info.gnum);
			that.parent().siblings('.t_total').text('￥'+info.this_total);
			$('.cart_total').text(info.total+'元');
	...
	// 购车商品数量减少
	$("a.cart_redg").click(function(){
		var key = $(this).parent('.changnum').attr('cartkey');
		var that = $(this); 
		$.get(change_cartgoods,{'addtype':'red','key':key},function(data){
			var info = eval(data);
			that.siblings('.t_cnum').text(info.gnum);
			that.parent().siblings('.t_total').text('￥'+info.this_total);
			$('.cart_total').text(info.total+'元');
	...
	// 马上结算
	$("a.js_btn").click(function(){

		if($("input[name=havesub]").val() == '1'){
			window.location.href='/index.php?g=App&m=Index&a=index&page_type=order';
		}
        ...
        if(!mobliepreg.test($("input[name=mobile]").val())){
            alert('手机号有误');
            return;
        }
		...
		$('input[name=havesub]').val('1');

		$.post(addorder,{'username':username,'mobile':mobile,'province':province,'city':city,'address':address,'remark':remark},function(data){
	...
	$("a.addcart").click(function(){
		...
		if(!$("p.gattr").hasClass('select') && $('input[name=attr]').val() != '0'){
			alert('请选择商品规格');
			return;
		}

		if(parseInt($('b.buynum').text()) >= 100){
			return false;
		}

		$('b.buynum').text(parseInt($('b.buynum').text())+1);


	});

	$("a.redcart").click(function(){
		if(parseInt($('b.buynum').text()) <= 1){
			return false;
		}
	...
	$("a.cart_btn").click(function(){
		...
		$.post(addcart,{'gid':$('input[name=gid]').val(),'attr':$('input[name=attr]').val(),'gnum':$('.buynum').text(),'gname':$('.gname').text()},function(data){
			var info = eval(data);
			if(info.cart_count){
				$('.cgnum').css('display','block');
				$('.cgnum').text(info.cart_count);
			}
