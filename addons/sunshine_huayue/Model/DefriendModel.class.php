...
class DefriendModel {
	...
	static function defriend($openid,$defriend_openid) {
		...
		return pdo_insert('sunshine_huayue_defriend',$data,true);
	}
	// relieve handle
	...
	static function info($openid,$defriend_openid) {
		...
		$r = pdo_fetch("select * from ".tablename('sunshine_huayue_defriend')." where uniacid={$_W['uniacid']} and openid='$openid' and defriend_openid='$defriend_openid'");
		return $r;
	}
	...
	static function lists($openid) {
		...
		return pdo_fetchall("select * from ".tablename('sunshine_huayue_defriend')." where uniacid={$_W['uniacid']} and openid='$openid' and status='y'");
	}
}