...
class ChatroomDefriendModel
{
	...
	*/
	static function defriend($room_id,$openid,$creator)
	{
		...
		return pdo_insert('sunshine_huayue_chatroom_defriend',$data,true);
	}
	/*
	从房间黑名单中移除
	*/
	...
	*/
	static function isDefriend($openid,$room_id)
	{
		...
		{
			$info = pdo_fetch("select * from ".tablename('sunshine_huayue_chatroom_defriend')." where uniacid={$_W['uniacid']} and room_id=$room_id and openid='$openid'");
