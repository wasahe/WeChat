var index_topscreen = (function(){
	...
	function drawGrowth(wrapper, value, sum, duration, isSvip, isAndroid) {
		...
		if (value <= sum/2) {
			...
			if (value < 0) {
				$left.css({'-webkit-transform': 'rotate(0deg) translateZ(0)'})
			} else {
				$left.css({
					'-webkit-transform': 'rotate('+((value/sum)*2*leftDegree+leftStartDegree)+'deg) translateZ(0)'
				})
			}
		}
		else {
			if (isAndroid) {
				...
				$right.css({
					'-webkit-transition-duration': 0,
					'-webkit-transform': 'rotate('+((value-sum/2)/(sum/2))*rightDegree+'deg) translateZ(0)'
				});
				if (value === sum) {
					$unit.addClass('full');
				}
