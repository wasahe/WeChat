...
class S
{
	static function timeDiff($time)
	{
		...
		if ($time >= 0 && $time < 60) {
			...
			$str = $str_h . "小时";
			$unit = '';
		} elseif ($time >= 24 * 60 * 60 && $time < 24 * 60 * 60 * 30) {
			$str_d = floor($time / 60 / 60 / 24);
			$str = $str_d . "天";
			$unit = '';
		} elseif ($time >= 24 * 60 * 60 * 30) {
			$str_d = floor($time / 60 / 60 / 24 / 30);
			$str = $str_d . "月";
			$unit = '';
		}
		return array('str' => $str, 'unit' => $unit);
	}
	static function timeDiffNew($time)
	{
		$unit = '秒';
		if ($time >= 0 && $time < 60) {
			$str = $time;
	...
	static function ConvertDistance($distance)
	{
		...
		return array('distance' => $distance, 'unit' => $unit);
	}
	static function userLevel($score)
	{
		$king = 0;
		$sun = 0;
		$moon = 0;
		$star = 0;
		if ($score >= 270) {
			$king = intval($score / 270);
			$score = $score - $king * 270;
		}
		if ($score >= 90 && $score < 270) {
			$sun = intval($score / 90);
			$score = $score - $sun * 90;
		}
		if ($score >= 30 && $score < 90) {
			$moon = intval($score / 30);
			$score = $score - $moon * 30;
		}
		$star = ceil($score / 10);
		$res = array();
	...
	static function downloadFromWxServer($media_ids, $settings)
	{
		...
		foreach ($media_ids as $id) {
			...
			$wr = file_write('/images/huayue/' . $filename, $data['content']);
			if ($wr) {
				$file_succ[] = array('name' => $filename, 'path' => '/images/huayue/' . $filename, 'type' => 'local');
			}
		}
		foreach ($file_succ as $key => $value) {
			if ($settings['qiniuallow'] == 'open') {
				$qiniu_r = self::upload2qiniu($value, $settings);
				if (!$qiniu_r) {
					unset($file_succ[$key]);
				} else {
					$file_succ[$key]['name'] = $qiniu_r;
					$file_succ[$key]['type'] = 'qiniu';
				}
			} else {
				$r = file_remote_upload('images/huayue/' . $value['name']);
				if (is_error($r)) {
					unset($file_succ[$key]);
					continue;
				}
				$file_succ[$key]['name'] = tomedia('images/huayue/' . $value['name']);
				$file_succ[$key]['type'] = 'other';
			}
		}
		return $file_succ;
	}
	static function upload2qiniu($file, $settings)
	{
		$filename = 'we7_sunshine_huayue/' . $file['name'];
		$filepath = ATTACHMENT_ROOT . '/images/huayue/' . $file['name'];
		require_once dirname(dirname(__FILE__)) . "/plugin/qiniu/io.php";
		require_once dirname(dirname(__FILE__)) . "/plugin/qiniu/rs.php";
		if (!$settings['qiniu_ak'] || !$settings['qiniu_sk'] || !$settings['qiniu_bucket']) {
			header("HTTP/1.1 500 Internal Server Error");
			echoJson(array("res" => '101', 'msg' => 'get qiniu settings error'));
		}
		$bucket = $settings['qiniu_bucket'];
		$accessKey = $settings['qiniu_ak'];
		$secretKey = $settings['qiniu_sk'];
		...
		return "http://" . $settings['qiniu_domain'] . "/" . $filename;
	}
}
