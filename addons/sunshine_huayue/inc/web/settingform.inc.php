...
if($_GPC['dataType'] == 'baidu_key')
{
	...
	$n = pdo_fetch("select count(*) as num from ".tablename('sunshine_huayue_setting')." where name='baidu_key' and uniacid={$_W['uniacid']}");
	if($n['num'])
	{
		$info = pdo_update("sunshine_huayue_setting",array('value'=>$_GPC['baidu_key']),array('name'=>'baidu_key','uniacid'=>$_W['uniacid']));
		if($info !== false)
		{
	...
	else
	{
		...
		$info = pdo_insert("sunshine_huayue_setting",$data,array('name'=>'baidu_key'));
		if($info)
		{
			echoJson(array('res'=>'100','msg'=>'success'));
		}
...
if($_GPC['dataType'] == 'oauth') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'oauth_appid','uniacid'=>$_W['uniacid']));
	$r2 = pdo_delete('sunshine_huayue_setting',array('name'=>'oauth_appsecret','uniacid'=>$_W['uniacid']));
	...
	$info_appid = pdo_insert("sunshine_huayue_setting",$data_appid);
	...
	$info_appsecret = pdo_insert("sunshine_huayue_setting",$data_appsecret);
...
if($_GPC['dataType'] == 'opengetuserinfo')
{
	...
	$r = pdo_delete('sunshine_huayue_setting',array('name'=>'opengetuserinfo','uniacid'=>$_W['uniacid']));
	if($r === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
	...
	$info_key = pdo_insert("sunshine_huayue_setting",$data_key);
...
if($_GPC['dataType'] == 'qiniuallow')
{
	...
	$r = pdo_delete('sunshine_huayue_setting',array('name'=>'qiniuallow','uniacid'=>$_W['uniacid']));
	if($r === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'qiniu_info') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'qiniu_ak','uniacid'=>$_W['uniacid']));
	$r2 = pdo_delete('sunshine_huayue_setting',array('name'=>'qiniu_sk','uniacid'=>$_W['uniacid']));
	$r3 = pdo_delete('sunshine_huayue_setting',array('name'=>'qiniu_bucket','uniacid'=>$_W['uniacid']));
	$r4 = pdo_delete('sunshine_huayue_setting',array('name'=>'qiniu_domain','uniacid'=>$_W['uniacid']));
	...
	$info_ak = pdo_insert("sunshine_huayue_setting",$data_ak);
	...
	$info_sk = pdo_insert("sunshine_huayue_setting",$data_sk);
	...
	$info_bucket = pdo_insert("sunshine_huayue_setting",$data_bucket);
	...
	$info_domain = pdo_insert("sunshine_huayue_setting",$data_domain);
...
if($_GPC['dataType'] == 'notice_id') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'notice_approve_tpl_id','uniacid'=>$_W['uniacid']));
	...
	$info_1 = pdo_insert("sunshine_huayue_setting",$data_1);
...
if($_GPC['dataType'] == 'notice_tpl') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'notice_first','uniacid'=>$_W['uniacid']));
	$r2 = pdo_delete('sunshine_huayue_setting',array('name'=>'notice_remark','uniacid'=>$_W['uniacid']));
	if($r1 === false || $r2 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
	...
	$info_2 = pdo_insert("sunshine_huayue_setting",$data_2);
...
if($_GPC['dataType'] == 'notice_key') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'notice_key','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'share_info') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'share_title','uniacid'=>$_W['uniacid']));
	$r2 = pdo_delete('sunshine_huayue_setting',array('name'=>'share_content','uniacid'=>$_W['uniacid']));
	if($r1 === false || $r2 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'super_title_set') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'super_title','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'filter') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'filter','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'filter_content') 
{
	...
	$file_path = dirname(dirname(dirname(dirname(__FILE__))))."/sunshine_huayue_config/";

	if(!is_dir($file_path)) {
		if(!mkdir($file_path)) {
			echoJson(array('res'=>'101','msg'=>'创建'.$file_path.'失败'));
		}
	}

	$path = $file_path.$_W['uniacid']."_filter.inc";
	clearstatcache();
	// if(!is_writable($path))
	// {
...
if($_GPC['dataType'] == 'signin_key') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'signin_key','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'signin_credit_save') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'signin_credit','uniacid'=>$_W['uniacid']));
	$r2 = pdo_delete('sunshine_huayue_setting',array('name'=>'signin_credit_grow','uniacid'=>$_W['uniacid']));
	if($r1 === false || $r2 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
	...
	$data_2['add_time'] = date("Y-m-d H:i:s");
	$info_1 = pdo_insert("sunshine_huayue_setting",$data_2);
...
if($_GPC['dataType'] == 'public_notice') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'public_notice','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'approve_id') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'approve_tpl_id','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'notice_time') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'notice_time','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'vip_credit') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'vip_credit_month_1','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'index_credit_jump_url') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'index_credit_jump_url','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'menu_fhzy_jump_url') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'menu_fhzy_jump_url','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'hall_tag') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'hall_tag_1','uniacid'=>$_W['uniacid']));
	$r2 = pdo_delete('sunshine_huayue_setting',array('name'=>'hall_tag_2','uniacid'=>$_W['uniacid']));
	if($r1 === false || $r2 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'robot_key') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'robot_key','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'robot_rate') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'robot_rate','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'robot_url') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'robot_url','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
if($_GPC['dataType'] == 'multi_send_url') 
{
	...
	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'multi_send_url','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
...
// 提现消息模板
if($_GPC['dataType'] == 'draw_wait_tpl_id') 
{
	$draw_wait_tpl_id = $_GPC['draw_wait_tpl_id'];

	pdo_begin();

	$r1 = pdo_delete('sunshine_huayue_setting',array('name'=>'draw_wait_tpl_id','uniacid'=>$_W['uniacid']));
	if($r1 === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
	...
	$data_1 = array();
	$data_1['uniacid'] = $_W['uniacid'];
	$data_1['name'] = 'draw_wait_tpl_id';
	$data_1['value'] = $_GPC['draw_wait_tpl_id'];
	$data_1['add_time'] = date("Y-m-d H:i:s");
	$info_1 = pdo_insert("sunshine_huayue_setting",$data_1);
...
// 语音下载开关
if($_GPC['dataType'] == 'voice_download')
{
	if($_GPC['voice_download'] != 'open' && $_GPC['voice_download'] != 'close')
	{
		echoJson(array('res'=>'101','msg'=>'error'));
	}
	...
	$r = pdo_delete('sunshine_huayue_setting',array('name'=>'voice_download','uniacid'=>$_W['uniacid']));
	if($r === false)
	{
		pdo_rollback();
		echoJson(array('res'=>'102','msg'=>'初始化数据失败'));
	...
	$data_key = array();
	$data_key['uniacid'] = $_W['uniacid'];
	$data_key['name'] = 'voice_download';
	$data_key['value'] = $_GPC['voice_download'];
	$data_key['add_time'] = date("Y-m-d H:i:s");
	$info_key = pdo_insert("sunshine_huayue_setting",$data_key);
