...
class CashierModel extends PluginModel
{
	...
	public function wechatpay_101($params)
	{
		...
		if (isset($set['pay']) && ($set['pay']['weixin'] == 1)) {
			...
			if (isset($set['pay']) && ($set['pay']['weixin_sub'] == 1)) {
				$sec = m('common')->getSec();
		...
		if (empty($wechat)) {
			...
			if (isset($set['pay']) && ($set['pay']['weixin_jie'] == 1)) {
				...
				if (isset($set['pay']) && ($set['pay']['weixin_jie_sub'] == 1)) {
	...
	public function pay($data, $auth_code = NULL)
	{
		...
		if (($data['money'] <= 0) || ($data['paytype'] == 3)) {
			...
			m('member')->setCredit($data['openid'], 'credit2', 0 - $data['deduction'], array(0, '收银台 ' . $_W['cashieruser']['title'], '收款'));
		...
		if (($res['total_fee'] == round($realmoney * 100, 2)) || ($res['total_amount'] == round($realmoney, 2))) {
			...
			if (empty($log['status'])) {
				...
				if (!empty($log['deduction'])) {
					...
					m('member')->setCredit($userinfo['openid'], 'credit2', 0 - $log['deduction'], array(0, '收银台 ' . $_W['cashieruser']['title'], '收款'));
				}
	...
	/**
     * @param $goods array('goodsid'=>525,'optionid'=>2939,'price'=>100,'total'=>1)
     * @param string $openid
     * @return mixed
     */
	...
	/**
     * 商品计算价格
     * @param array $selfgoods 本地收银台商品
     * @param array $shopgoods 商城商品
     */
