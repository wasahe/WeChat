...
class Data_EweiShopV2Page extends ComWebPage
{
	...
	public function main()
	{
		...
		if (empty($typeid)) {
			$this->message('Url参数错误！请重试！', webUrl('virtual/temp'), 'error');
			exit();
		}
		...
		unset($row);
		$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_virtual_data') . ' d ' . ' left join ' . tablename('ewei_shop_member') . ' m on m.openid = d.openid and m.uniacid=d.uniacid and d.merchid=0' . ' left join ' . tablename('ewei_shop_order') . ' o on o.id = d.orderid ' . ' where 1 ' . $condition . ' ', $params);
	...
	protected function post()
	{
		...
		$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and merchid=0', array(':id' => $_GPC['typeid'], ':uniacid' => $_W['uniacid']));
		...
		if (!empty($editid)) {
			$data = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_data') . ' WHERE id=:id and typeid=:typeid and uniacid=:uniacid and merchid=0', array(':id' => $editid, ':typeid' => $typeid, ':uniacid' => $_W['uniacid']));
			$data['edit'] = $editid;
		}
		...
		if ($_W['ispost']) {
			...
			if (!empty($typeid)) {
				$item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and merchid=0', array(':id' => $typeid, ':uniacid' => $_W['uniacid']));
				$item['fields'] = iunserializer($item['fields']);
				...
				if (!empty($item['fields'])) {
					...
					foreach ($tpids as $index => $id) {
						...
						if (empty($datas)) {
							$keydata = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_data') . ' WHERE pvalue=:pvalue  and typeid=:typeid and uniacid=:uniacid and merchid=0', array(':pvalue' => $insert['pvalue'], ':typeid' => $typeid, ':uniacid' => $_W['uniacid']));
							...
							$keydata = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_data') . ' WHERE pvalue=:pvalue and id<>:id and typeid=:typeid and uniacid=:uniacid and merchid=0', array(':pvalue' => $insert['pvalue'], ':id' => $id, ':typeid' => $typeid, ':uniacid' => $_W['uniacid']));
					...
					plog('goods.virtual.data.edit', '修改数据 模板ID: ' . $typeid);

					if (!empty($noinsert)) {
						$tip = '<br>未保存成功的数据：主键=' . $noinsert . '<br>失败原因：已经使用无法更改';
						show_json(1, array('message' => '部分数据保存成功！' . $tip, 'url' => webUrl('virtual/data', array('typeid' => $typeid))));
					}
					else {
						show_json(1, array('url' => webUrl('goods/virtual/data', array('typeid' => $typeid))));
					}
				}
			}
		}
	...
	public function delete()
	{
		...
		$types = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_virtual_data') . ' WHERE id in( ' . $id . ' ) AND typeid=' . $typeid . ' and merchid=0 and uniacid=' . $_W['uniacid']);
		...
		foreach ($types as $type) {
			...
			plog('goods.virtual.data.delete', '删除数据 模板ID: ' . $typeid . ' 数据ID: ' . $id);
		}
	...
	public function export()
	{
		...
		global $_GPC;
		$typeid = intval($_GPC['typeid']);
		$type = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_type') . ' WHERE id=:id and uniacid=:uniacid and merchid=0 limit 1 ', array(':id' => $typeid, ':uniacid' => $_W['uniacid']));
		...
		unset($row);
		$columns = array(
			array('title' => $fieldstr, 'field' => 'values', 'width' => 24),
			...
			array('title' => '购买价格', 'field' => 'price', 'width' => 12)
			);
