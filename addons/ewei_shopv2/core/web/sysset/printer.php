...
class Printer_EweiShopV2Page extends WebPage
{
	...
	protected function post()
	{
		...
		if ($_W['ispost']) {
			$data = array('uniacid' => $_W['uniacid'], 'type' => intval($_GPC['type']), 'title' => trim($_GPC['title']), 'print_title' => trim($_GPC['print_title']), 'print_style' => trim($_GPC['print_style']), 'code' => intval($_GPC['code']), 'qrcode' => trim($_GPC['qrcode']));
	...
	protected function printer_post()
	{
		...
		if ($_W['ispost']) {
			$data = array('uniacid' => $_W['uniacid'], 'type' => intval($_GPC['type']), 'title' => trim($_GPC['title']));
