...
class Printer_EweiShopV2ComModel extends ComModel
{
	...
	{
		return array(self::PRINTRT_365_S1 => '365云打印(编号kdt1)', self::PRINTRT_365 => '365云打印(编号kdt2)', self::PRINTRT_YILIANYUN => '易联云K1、K2、K3', self::PRINTRT_YILIANYUN_NEW => '易联云K2S、K3S、K4、M1', self::PRINTRT_FEIE => '飞鹅打印机');
	}
	...
	public function sendOrderMessage($orderid = 0, $verify = array())
	{
		...
		foreach ($order_goods as $og) {
			$goods[] = array('shorttitle' => empty($og['shorttitle']) ? $og['title'] : $og['shorttitle'], 'goodsprice' => (double) $og['price'] / (int) $og['total'], 'goodstotal' => (int) $og['total'], 'goodstotalprice' => (double) $og['price'], 'optiontitle' => $og['optiontitle']);
		}
