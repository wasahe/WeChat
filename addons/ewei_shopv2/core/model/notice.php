...
class Notice_EweiShopV2Model
{
	...
	public function sendTeamMessage($orderid = '0', $delRefund = false)
	{
		...
		$goods = ' (单价: yen' . $goodsprice . '元 数量: 1 总价: yen' . $order['price'] . '元); ';
		...
		if ($delRefund == true) {
			...
			$msg = array(
				...
				'keyword1' => array('title' => '退款金额', 'value' => 'yen' . $price . '元', 'color' => '#000000'),
				...
				'remark'   => array('value' => '退款金额 yen' . $price . $refundtype . "\r\n 【" . $shop['name'] . '】期待您再次购物！', 'color' => '#000000')
				);
					...
					if (($order['status'] == 2) || (($order['status'] == 1) && !empty($order['sendtype']))) {
						if (empty($order['dispatchtype'])) {
							...
							if (empty($order['sendtype'])) {
								...
								$text = "您的宝贝已经成功发货！ \n商品名称：[商品详情]\n快递公司：[快递公司]\n快递单号：[快递单号]\n" . $remark;
								$msg = array(
									'first'    => array('value' => '您的宝贝已经发货！', 'color' => '#000000'),
									'keyword1' => array('title' => '订单编号', 'value' => $order['ordersn'], 'color' => '#000000'),
									'keyword2' => array('title' => '快递公司', 'value' => $order['expresscom'], 'color' => '#000000'),
									'keyword3' => array('title' => '快递单号', 'value' => $order['expresssn'], 'color' => '#000000'),
							...
							if ($pv && !empty($order['virtual'])) {
								if (empty($usernotice['virtualsend'])) {
									...
									$msg = array(
										...
										'keyword3' => array('title' => '订单金额', 'value' => 'yen' . $order['price'] . '元', 'color' => '#000000'),
	...
	public function sendMemberUpgradeMessage($openid = '', $oldlevel = NULL, $level = NULL)
	{
		...
		$this->sendNotice(array('openid' => $openid, 'tag' => 'upgrade', 'default' => $message, 'cusdefault' => $text, 'url' => $detailurl, 'datas' => $datas));
		...
		if ($log_info['type'] == 0) {
			if ($log_info['status'] == 1) {
				if ($isback) {
					...
					$this->sendNotice(array('openid' => $log_info['openid'], 'tag' => 'backrecharge_ok', 'default' => $message, 'cusdefault' => $text, 'url' => $url, 'datas' => $datas));
