...
class Api
{
     ...
     * @return array | error
     */
	public function payOrder($openid, $order, $config = array())
	{
		$pars = array();
		...
		if (!empty($order['uniacid'])) {
			$pars['attach'] = $order['uniacid'];
		}


		$pars['out_trade_no'] = $order['order_sn'];
		$pars['total_fee'] = $order['payment'] * 100;
		$pars['spbill_create_ip'] = get_client_ip(0, true);
		$pars['notify_url'] = $config['notify_url'];
	...
	static private function getAccessToken($appid, $secret)
	{
		...
		if (empty($token) || !is_array($token) || empty($token['access_token']) || empty($token['expires_in'])) {
			...
			return error($errorinfo['errcode'], '获取微信公众号授权失败, 请稍后重试！ 公众平台返回原始数据为: 错误代码-' . $errorinfo['errcode'] . '，错误信息-' . $errorinfo['errmsg']);
		}
	...
	{
		$access = $this->core->module['config']['access'];

		if (!empty($access) && !empty($access['token']) && (time() < $access['expire'])) {
			return $access['token'];
		}
	...
	public function push($openid, $packet)
	{
		...
		if ($packet['type'] == Platform::PACKET_NEWS) {
			...
			if (10 < count($packet['news'])) {
				$packet['news'] = array_rand($packet['news'], 10);
			}


			foreach ($packet['news'] as $article ) {
				$pars['news']['articles'][] = array('title' => urlencode($article['title']), 'description' => urlencode($article['description']), 'url' => $article['url'], 'picurl' => $article['image']);
			}
		}
	...
	public function shareDataCreate($forceUrl = '')
	{
		...
		if (is_array($ticket) && !empty($ticket['ticket']) && !empty($ticket['expire']) && (TIMESTAMP < $ticket['expire'])) {
			$t = $ticket['ticket'];
		}
		 else {
			$token = $this->fetchToken();
