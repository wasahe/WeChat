...
class Activity{
    ...
    public function create($entity){
        global $_W;
        $rec = array_elements(array('title','description','shareimg','content','singleimg','status','extype','starttime','endtime','place','proname','tlimit','scantimes','buylimit','fee','author','groups','viewnums','isseat','seatsets','usedseats','orderseats'), $entity);
    ...
    public function getlimitseats($actid){
        $act = $this->getOne($actid);
        $limitseats = unserialize($act['usedseats']);
        $limitseats = str_replace("&#039;","'" ,$limitseats );
        $lseats = array_filter(explode(',',$limitseats));
        if(!is_array($lseats) || count($lseats) < 1){
           return false;
        }
        return $lseats;
    }
