...
global $_W, $_GPC;
$foo = $_GPC['foo'];
$foos = array('list', 'create', 'modify', 'delete', 'order','change','log','seat');
...
require_once HK_ROOT . '/module/Seat.class.php';
$a = new Activity();
$o = new Order();
$l = new Log();
$s = new Seat();
...
$group = pdo_fetchall('SELECT groupid,title FROM ' . tablename('mc_groups') . " WHERE uniacid = '{$_W['uniacid']}' ");
    if($_W['ispost']) {
        ...
        if($input['isseat']){
            $seata=base64_encode($input['seattype']);
            $seatb=base64_encode($input['crset']);
            $seatc=base64_encode($input['seatset']);
            $input['seatsets']=serialize(array($seata,$seatb,$seatc));
            $input['usedseats'] = serialize($input['usedseats']);
        }
        $ret = $a->create($input);
        if(is_error($ret)) {
            message($ret['message']);
...
if($foo == 'modify') {
    ...
    if($activity['isseat']){
        $atmp=unserialize($activity['seatsets']);
        $activity['seattype']=base64_decode($atmp[0]);
        $activity['crset']=base64_decode($atmp[1]);
        $activity['seatset']=base64_decode($atmp[2]);
        $activity['usedseats'] = unserialize($activity['usedseats']);
    }
...
if($foo == 'order') {
    ...
    if(is_array($ds)){
        foreach ($ds as $k=>&$v){
            //加入座位 www.efwww.com  易福源码网
            $sdata = $s->getOne($v['id']);
            $tsarry = explode(',',$sdata['seats']);
            foreach ($tsarry as $k=>&$vv){
                $vv = trim($vv,"'");
            }
            $v['seats']=implode(',',$tsarry);
