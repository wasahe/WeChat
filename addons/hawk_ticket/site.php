	...
	public function payResult($params) {
		...
		if ($params['result'] == 'success') {
			...
			if($actinfo['isseat']){
				$sinfo  = $scl->getOne($id);
				$useats = $sinfo['seats'];
				$iseats = explode(',',$useats);
				if(empty($actinfo['orderseats'])){
					$rseats['orderseats'] = base64_encode(serialize($iseats));
				}else{
					$oseats = unserialize(base64_decode($actinfo['orderseats']));
					$rseats['orderseats']= base64_encode(serialize(array_merge($oseats,$iseats)));

				}
				$act->modify($actinfo['id'],$rseats);
			}
