...
class QRResponser
{
	public function aa($openid)
	{
		global $_W;
		$uniacid = $_W['uniacid'];
		$member = pdo_fetch('select * from ' . tablename('hc_deluxejjr_member') . ' where uniacid = ' . $uniacid . ' and openid = \'' . $openid . '\'');
		$id = $member['id'];
		$imgname = $uniacid . 'share' . $id . '.png';
		$qr_file = IA_ROOT . '/addons/hc_deluxejjr/style/poster/membershare/' . $imgname;
		if (!file_exists($qr_file)) {
			require IA_ROOT . '/framework/library/qrcode/phpqrcode.php';
			$value = $_W['siteroot'] . 'app/index.php?i=' . $uniacid . '&c=entry&m=hc_deluxejjr&do=register&mid=' . $id;
			$errorCorrectionLevel = 'L';
			$matrixPointSize = '4';
			QRcode::png($value, $qr_file, $errorCorrectionLevel, $matrixPointSize);
		}
		$qr_file = $_W['siteroot'] . '/addons/hc_deluxejjr/style/poster/membershare/' . $imgname;
		$poster = pdo_fetch('SELECT * FROM ' . tablename('hc_deluxejjr_poster') . ' WHERE uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
		...
		foreach ($alldata as $dat) {
			...
			if ($dat['type'] == 'head') {
				$target = $this->mergeImage($target, $dat, $member['headurl']);
			} else {
				if ($dat['type'] == 'img') {
		...
		if (file_exists($target_file)) {
			...
			if (!empty($media_id)) {
				pdo_update('hc_deluxejjr_member', array('media_id' => $media_id, 'ischange' => 0, 'media_time' => time()), array('id' => $id));
	...
	{
		$qr_url = NULL;
		$data = array('action_name' => 'QR_LIMIT_SCENE', 'action_info' => array('scene' => array('scene_id' => intval($scene_id))));
	...
	{
		$font = IA_ROOT . '/addons/hc_deluxejjr/style/font/qrcodefont/msyhbd.ttf';
	...
	public function create_image($imgurl)
	{
