var card = {
    ...
    //添加经纪人信息
    hc_deluxejjrradd: function (btn, A1, name, phone, hc_deluxejjrrType, UnitNumber, hc_deluxejjrr) {
        var $sub = $(btn);
        var $A1 = $(A1);
        var $name = $(name);
        var $phone = $(phone);
        var $hc_deluxejjrrType = $(hc_deluxejjrrType);
        var $UnitNumber = $(UnitNumber);
        var $hc_deluxejjrr = $(hc_deluxejjrr);
        if ($sub.length > 0 && $phone.length > 0) {
            $sub.live("click", function () {
                var reg = /^1[34578]\d{9}$/;
                ...
                if (n == "") {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/tools/tool.ashx",
                        data: { action: "addcard", mtype: 2, name: encodeURIComponent(n), phone: p, op: encodeURIComponent(card.OpenId), type: t, unitnumber: u, hc_deluxejjrr: b },
                        dataType: "text",
                        //提交服务器成功
                        success: function (d) {
                            if (d == "1") {
                                $name.val("");
                                $phone.val("");
                                TopBox.alert("经纪人申请成功.", function () { window.location.href = './hc_deluxejjrrIndex.aspx?code=' + card.code; });
                            } else if (d == "0") {
                                $sub.show(); $A1.hide();
    ...
    //推荐客户
    hc_deluxejjrrClientAdd: function (btn, name, phone) {
        var $sub = $(btn);
        var $name = $(name);
        var $phone = $(phone);
        if ($sub.length > 0 && $phone.length > 0) {
            $sub.live("click", function () {
                ...
                if (n == "") {
                    ...
                    $.ajax({
                        ...
                        data: { action: "addhc_deluxejjrrClient", name: encodeURIComponent(n), phone: p, op: encodeURIComponent(card.OpenId) },
                        dataType: "text",
                        //提交服务器成功
                        success: function (d) {
                            if (d == "1") {
