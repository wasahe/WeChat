...
if ($op == 'lpcustomernum') 
{
	...
	if ($pp == 'xml') 
	{
		...
		if (!empty($loupanids)) 
		{
			$logs = pdo_fetchall('select count(id) as num, loupan from ' . tablename('hc_deluxejjr_customer') . ' where loupan in ' . $loupanid . ' and uniacid = ' . $uniacid . ' and createtime >= ' . $starttime . ' and createtime <' . $endtime . ' group by loupan');
		}
		else 
		{
			$logs = pdo_fetchall('select count(id) as num, loupan from ' . tablename('hc_deluxejjr_customer') . ' where uniacid = ' . $uniacid . ' and createtime >= ' . $starttime . ' and createtime <' . $endtime . ' group by loupan');
		}
...
if ($op == 'arealoupan') 
{
	...
	if ($pp == 'xml') 
	{
		if (empty($location_p)) 
		{
			$logs = pdo_fetchall('select count(id) as num, location_p as location from ' . tablename('hc_deluxejjr_loupan') . ' where uniacid = ' . $uniacid . ' and createtime >= ' . $starttime . ' and createtime <' . $endtime . ' group by location_p');
		}
		else if (empty($location_c)) 
		{
			$logs = pdo_fetchall('select count(id) as num, location_c as location from ' . tablename('hc_deluxejjr_loupan') . ' where location_p = \'' . $location_p . '\' and uniacid = ' . $uniacid . ' and createtime >= ' . $starttime . ' and createtime <' . $endtime . ' group by location_c');
		}
		else if (empty($location_a)) 
		{
			$logs = pdo_fetchall('select count(id) as num, location_a as location from ' . tablename('hc_deluxejjr_loupan') . ' where location_p = \'' . $location_p . '\' and location_c = \'' . $location_c . '\' and uniacid = ' . $uniacid . ' and createtime >= ' . $starttime . ' and createtime <' . $endtime . ' group by location_c');
		}
		else 
		{
			$logs = pdo_fetchall('select count(id) as num, location_a as location from ' . tablename('hc_deluxejjr_loupan') . ' where location_p = \'' . $location_p . '\' and location_c = \'' . $location_c . '\' and location_a = \'' . $location_a . '\' and uniacid = ' . $uniacid . ' and createtime >= ' . $starttime . ' and createtime <' . $endtime . ' group by location_c');
		}
		$string = '<?xml version="1.0" encoding="utf-8"?>' . "\r\n\t\t\t\t" . '<chart caption="">' . "\r\n\t\t\t\t" . '</chart>';
		$xml = simplexml_load_string($string);
		$xml['caption'] = $caption;
...
if ($op == 'loupansuccess') 
{
	...
	if ($pp == 'xml') 
	{
		...
		if (!empty($loupanids)) 
		{
			$logs = pdo_fetchall('select count(id) as num, loupan from ' . tablename('hc_deluxejjr_customer') . ' where status = ' . $success_key . ' and loupan in ' . $loupanid . ' and uniacid = ' . $uniacid . ' and createtime >= ' . $starttime . ' and createtime <' . $endtime . ' group by loupan');
		}
		else 
		{
			$logs = pdo_fetchall('select count(id) as num, loupan from ' . tablename('hc_deluxejjr_customer') . ' where status = ' . $success_key . ' and uniacid = ' . $uniacid . ' and createtime >= ' . $starttime . ' and createtime <' . $endtime . ' group by loupan');
		}
...
if ($op == 'cusstatus') 
{
	...
	if ($pp == 'xml') 
	{
		...
		if (!empty($loupanids)) 
		{
			$logs = pdo_fetchall('select count(id) as num, status, loupan from' . tablename('hc_deluxejjr_customer') . 'where loupan in ' . $loupanid . ' and uniacid =' . $uniacid . '.and updatetime >= ' . $starttime . '.and updatetime <' . $endtime . '.group by status');
		}
		else if (empty($lid)) 
		{
			$logs = pdo_fetchall('select count(id) as num, status, loupan from' . tablename('hc_deluxejjr_customer') . 'where uniacid =' . $uniacid . '.and updatetime >= ' . $starttime . '.and updatetime <' . $endtime . '.group by status');
		}
		else 
		{
			$logs = pdo_fetchall('select count(id) as num, status, loupan from' . tablename('hc_deluxejjr_customer') . 'where loupan = ' . $lid . ' and uniacid =' . $uniacid . '.and updatetime >= ' . $starttime . '.and updatetime <' . $endtime . '.group by status');
		}
		$string = '<?xml version="1.0" encoding="utf-8"?>' . "\r\n\t\t\t\t" . '<chart caption="">' . "\r\n\t\t\t\t" . '</chart>';
		$xml = simplexml_load_string($string);
		$xml['caption'] = $caption;
...
if ($op == 'customer') 
{
	...
	if ($pp == 'xml') 
	{
		...
		if (!empty($loupanids)) 
		{
			$logs = pdo_fetchall('select count(id) as num, createtime1 from' . tablename('hc_deluxejjr_customer') . 'where loupan in ' . $loupanid . ' and uniacid =' . $uniacid . '.and createtime >= ' . $starttime . '.and createtime <' . $endtime . '.group by createtime1 order by createtime asc');
		}
		else if (empty($lid)) 
		{
			$logs = pdo_fetchall('select count(id) as num, createtime1 from' . tablename('hc_deluxejjr_customer') . 'where uniacid =' . $uniacid . '.and createtime >= ' . $starttime . '.and createtime <' . $endtime . '.group by createtime1 order by createtime asc');
		}
		else 
		{
			$logs = pdo_fetchall('select count(id) as num, createtime1 from' . tablename('hc_deluxejjr_customer') . 'where loupan = ' . $lid . ' and uniacid =' . $uniacid . '.and createtime >= ' . $starttime . '.and createtime <' . $endtime . '.group by createtime1 order by createtime asc');
		}
		$string = '<?xml version="1.0" encoding="utf-8"?>' . "\r\n\t\t\t\t" . '<chart caption="" lineThickness="4" showValues="1" formatNumberScale="0" anchorRadius="4" divLineAlpha="15" divLineColor="666666" divLineIsDashed="1" showAlternateHGridColor="1" alternateHGridColor="666666" shadowAlpha="40" labelStep="2" numvdivlines="5" chartRightMargin="35" bgColor="FFFFFF,FFFFFF" bgAngle="270" bgAlpha="10,10" alternateHGridAlpha="5" legendPosition="RIGHT " baseFontSize="12" baseFont="Microsoft YaHei,Helvitica,Verdana,Arial,san-serif" canvasBorderThickness="1" canvasBorderColor="888888" showShadow="1" animation="1" showBorder="0" showToolTip="1" adjustDiv="1" setAdaptiveYMin="1" defaultAnimation="1">' . "\r\n\t\t\t\t\t" . '<categories>' . "\r\n\t\t\t\t\t\t\r\n\t\t\t\t\t" . '</categories>' . "\r\n\t\t\t\t\t" . '<dataset seriesName="" color="66cc00" anchorBorderColor="66cc00" anchorBgColor="ffffff">' . "\r\n\t\t\t\t\t\t\r\n\t\t\t\t\t" . '</dataset>' . "\r\n\t\t\t\t\t" . '<styles>' . "\r\n\t\t\t\t\t\t" . '<definition>' . "\r\n\t\t\t\t\t\t\t" . '<style name="CaptionFont" type="font" size="12"/>' . "\r\n\t\t\t\t\t\t" . '</definition>' . "\r\n\t\t\t\t\t\t" . '<application>' . "\r\n\t\t\t\t\t\t\t" . '<apply toObject="CAPTION" styles="CaptionFont"/>' . "\r\n\t\t\t\t\t\t\t" . '<apply toObject="SUBCAPTION" styles="CaptionFont"/>' . "\r\n\t\t\t\t\t\t" . '</application>' . "\r\n\t\t\t\t\t" . '</styles>' . "\r\n\t\t\t\t" . '</chart>';
		$xml = simplexml_load_string($string);
		$xml['caption'] = $caption;
	...
	else 
	{
		if (!empty($loupanids)) 
		{
			$customers = pdo_fetchall('select status from' . tablename('hc_deluxejjr_customer') . 'where loupan in ' . $loupanid . ' and uniacid =' . $uniacid . '.and createtime >= ' . $starttime . '.and createtime <' . $endtime);
		}
		else if (empty($lid)) 
		{
			$customers = pdo_fetchall('select status from' . tablename('hc_deluxejjr_customer') . 'where uniacid =' . $uniacid . '.and createtime >= ' . $starttime . '.and createtime <' . $endtime);
		}
		else 
		{
			$customers = pdo_fetchall('select status from' . tablename('hc_deluxejjr_customer') . 'where loupan = ' . $lid . ' and uniacid =' . $uniacid . '.and createtime >= ' . $starttime . '.and createtime <' . $endtime);
		}
		$status = $this->ProcessStatus();
		$customernum = 0;
		$status1num = 0;
...
if ($op == 'activecus') 
{
	...
	if ($pp == 'xml') 
	{
		...
		if (!empty($loupanids)) 
		{
			$logs = pdo_fetchall('select count(id) as num, identity from' . tablename('hc_deluxejjr_customer') . 'where loupan in ' . $loupanid . ' and openid != \'\' and identity != 0 and uniacid =' . $uniacid . '.and createtime >= ' . $starttime . '.and createtime <' . $endtime . '.group by identity');
		}
		else if (empty($lid)) 
		{
			$logs = pdo_fetchall('select count(id) as num, identity from' . tablename('hc_deluxejjr_customer') . 'where uniacid =' . $uniacid . ' and openid != \'\' and identity != 0 and createtime >= ' . $starttime . '.and createtime <' . $endtime . '.group by identity');
		}
		else 
		{
			$logs = pdo_fetchall('select count(id) as num, identity from' . tablename('hc_deluxejjr_customer') . 'where loupan = ' . $lid . ' and openid != \'\' and identity != 0 and uniacid =' . $uniacid . '.and createtime >= ' . $starttime . '.and createtime <' . $endtime . '.group by identity');
		}
		$string = '<?xml version="1.0" encoding="utf-8"?>' . "\r\n\t\t\t\t" . '<chart caption="">' . "\r\n\t\t\t\t" . '</chart>';
		$xml = simplexml_load_string($string);
		$xml['caption'] = $caption;
...
if ($op == 'activemem') 
{
	...
	if ($pp == 'xml') 
	{
		...
		if (!empty($loupanids)) 
		{
			$logs = pdo_fetchall('select count(distinct openid) as num, identity from' . tablename('hc_deluxejjr_customer') . 'where loupan in ' . $loupanid . ' and openid != \'\' and identity != 0 and uniacid =' . $uniacid . '.and createtime >= ' . $starttime . '.and createtime <' . $endtime . '.group by identity');
		}
		else if (empty($lid)) 
		{
			$logs = pdo_fetchall('select count(distinct openid) as num, identity from' . tablename('hc_deluxejjr_customer') . 'where uniacid =' . $uniacid . ' and openid != \'\' and identity != 0 and createtime >= ' . $starttime . '.and createtime <' . $endtime . '.group by identity');
		}
		else 
		{
			$logs = pdo_fetchall('select count(distinct openid) as num, identity from' . tablename('hc_deluxejjr_customer') . 'where loupan = ' . $lid . ' and openid != \'\' and identity != 0 and uniacid =' . $uniacid . '.and createtime >= ' . $starttime . '.and createtime <' . $endtime . '.group by identity');
		}
		$string = '<?xml version="1.0" encoding="utf-8"?>' . "\r\n\t\t\t\t" . '<chart caption="">' . "\r\n\t\t\t\t" . '</chart>';
		$xml = simplexml_load_string($string);
		$xml['caption'] = $caption;
