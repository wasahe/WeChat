...
require_once IA_ROOT . '/addons/hc_deluxejjr/responser.php';
require_once IA_ROOT . '/addons/hc_deluxejjr/wechatutil.php';

class hc_deluxejjrModuleProcessor extends WeModuleProcessor {
	...
	public function respond() {
		...
		$id = $member['id'];
		if(!intval($id)){
			return $this->respText('您还未注册成为经纪人，请先注册后再来获取专属名片！');
		}
		...
		if(empty($member['media_id'])){
			...
			if(!file_exists($target_file)){
				...
				pdo_update('hc_deluxejjr_member', array('media_id'=>$media_id, 'ischange'=>0, 'mediatime'=>time()), array('id'=>$id));
