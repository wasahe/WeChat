...
public function postXmlCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, $this->setting['certpath']);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, $this->setting['keypath']);
		//post提交方式
...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	function createXml()
	{
	   	...
	   	$this->parameters["appid"] = $this->setting['appid'];//公众账号ID
	   	$this->parameters["mch_id"] =$this->setting['mchid'];//商户号
	    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
	    $this->parameters["sign"] = $this->getSign($this->parameters,$this->setting['shkey']);//签名
...
class Wxpay_server_pub extends Common_util_pub 
{
	...
	function checkSign($shkey)
	{
		...
		$tmpData = $this->data;
		unset($tmpData['sign']);
		$sign = $this->getSign($tmpData,$shkey);//本地签名
