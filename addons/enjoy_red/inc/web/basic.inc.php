...
if(checksubmit('submit')){
	...
	$data=array(
			...
			'city'=>trim($_GPC['city']),
			'locationtype'=>intval($_GPC['locationtype']),
			'color'=>$_GPC['color'],
			'apic'=>$_GPC['apic'],
			'fpic'=>$_GPC['fpic'],
			'bgpic'=>$_GPC['bgpic'],
...
// 			'prize_tel'=>$_GPC['prize_tel'],
			'stime'=>$_GPC['stime'],
			'etime'=>$_GPC['etime'],
			'cashstime'=>$_GPC['cashstime'],
			'cashetime'=>$_GPC['cashetime'],
	);
	if($exist>0){
		//update
		$res=pdo_update('enjoy_red_reply',$data,array('uniacid'=>$uniacid));
		$message="更新活动成功";
