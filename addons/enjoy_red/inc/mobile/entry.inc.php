...
$puid=intval($_GET['puid']);
	//授权登录，获取粉丝信息
$user = $this->auth($puid);
//  $user['openid']="oH3mWwElKnt_L_afW44fKyP2z9rw";
//  $user['uniacid']="2";
$userinfo1 = $_W['fans'];
//$userinfo = mc_oauth_userinfo();
//$userlist=pdo_fetch("select * from ".tablename('enjoy_goods_fans')." where uniacid=".$user['uniacid']." and openid='".$user['openid']."'");

if($userinfo1['follow']!=$user['subscribe']){
	pdo_update("enjoy_red_fans",array('subscribe'=>$userinfo1['follow']),array('uniacid'=>$user['uniacid'],'openid'=>$user['openid']));
}
	//取活动信息
...
if(empty($puid)){
	...
	if($puid!=$user['uid']){
		...
		if($shareC>0){
			...
			//判断前台是否单向传递
			if($actdetail['oneway']==1){
				//<1表示没翻到金额
	//$fcount=pdo_fetchcolumn("select count(*) from ".tablename('enjoy_red_chance_log')." where openid='".$user['openid']."' and uniacid=".$user['uniacid']."");
					
				//找到puid对应的openid和当前用户的uid
	$openid=pdo_fetchcolumn("select openid from ".tablename('enjoy_red_fans')." where uniacid=".$user['uniacid']." and uid=".$puid."");
	$uid=$user['uid'];
	$sharecount=pdo_fetchcolumn("select count(*) from ".tablename('enjoy_red_chance_log')." where uniacid=".$user['uniacid']." and puid=".$uid." and openid='".$openid."'");
	if($sharecount<1&&$user['puid']==$puid){
			//查询这个人已经有的记录
			$count1=pdo_fetchcolumn("select count(*) from ".tablename('enjoy_red_chance_log')." where uniacid=".$user['uniacid']." and puid=".$puid."");
            //查询这个人剩余的机会
				$chance=pdo_fetchcolumn("select chance from ".tablename('enjoy_red_chance')." where uniacid=".$user['uniacid']." and openid='".$popenid."'");
			$count=$count1+$chance;
			if($count<=$actdetail['times']-1){
			...
			pdo_update('enjoy_red_chance',array('chance'=>$chance),array('uniacid'=>$user['uniacid'],'openid'=>$popenid));
			//插入机会表
			$data=array(
			...
			pdo_insert('enjoy_red_chance_log',$data);
				...
				//查询这个人已经有的记录
				$count=pdo_fetchcolumn("select count(*) from ".tablename('enjoy_red_chance_log')." where uniacid=".$user['uniacid']." and puid=".$puid."");
...
//分享信息
$sharelink =  $_W['siteroot'] . "app/".$this->createMobileUrl('entry',array('puid'=>$user['uid']));
