...
class Truein_jianduModuleSite extends WeModuleSite {
	...
	public function doMobileIndex() {
		...
		if($data['openid']){
			...
			if(empty($staff)){
				...
				$data['headimgurl'] = $_W['fans']['tag']['avatar'];
				$data['sex'] = $_W['fans']['tag']['sex'];
				$data['country'] = $_W['fans']['tag']['country'];
				$data['province'] = $_W['fans']['tag']['province'];
	...
	public function doMobileShareinfo(){
		...
		truein_logs($data);
		//贴广告
		$url = $_W['siteroot'] . 'app/' . $this->createMobileUrl('adclick', array('sid' => $sid), true);
		$res = truein_getContent1($sid,$url,$accinfo);
		if(!$res['status']){
	...
	public function doMobileAdclick(){
		...
		//更新share表
		$logsdate = pdo_fetchcolumn('SELECT datetime FROM ' . tablename("truein_jiandu_logs") . ' WHERE share_id=:sid AND from_openid=:openid AND `click`=1 order by datetime desc LIMIT 1', array(':sid' => $data['sid'],':openid' => $_COOKIE['openid']));
