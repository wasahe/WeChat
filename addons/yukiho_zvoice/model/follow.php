...
class follow
{
    public $table = 'yukiho_zvoice_follow';
    ...
    public function deleteFollow($openid,$to_openid){
        if(empty($openid)){
            return '';
        }
        if(empty($to_openid)){
        ...
        pdo_delete($this->table,array('openid'=>$openid,'to_openid'=>$to_openid));
    }    
	
	public function deleteFollows($to_openid){
        if(empty($to_openid)){
            return '';
        }
        pdo_delete($this->table,array('to_openid'=>$to_openid));
    }
    ...
    public function getList($page,$where ="",$params = array()){
        ...
        $p = trim($_GPC['to_openid']);
        if(!empty($p)){
            $where .= " AND to_openid = :to_openid";
            $params[':to_openid'] = $p;
        }
        unset($p);
    ...
    public function update($data){
        ...
        if(empty($data['id'])){
            if(!empty($data['openid']) && !empty($data['to_openid'])){
    ...
    public function check($openid){
        global $_W;
		if($_W['openid']==$openid) return true;
        $item = pdo_get($this->table,array('openid'=>$_W['openid'],'to_openid'=>$openid));
        return $item;
    }
    public function getTotal($to_openid){
        global $_W;
        $sql = "SELECT COUNT(*) FROM ".tablename($this->table)." WHERE to_openid = :openid AND uniacid = :uniacid ";
        $params = array(':openid'=>$to_openid,':uniacid'=>$_W['uniacid']);
