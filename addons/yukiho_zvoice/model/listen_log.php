...
class listen_log
{
    ...
    public function deleteBySn($sn){
        if(empty($sn)){
            return '';
        }
        pdo_delete($this->table,array('sn'=>$sn));
    }
    ...
    public function gettotal($question_id,$type='question'){
        $sql = "SELECT COUNT(*) FROM ".tablename($this->table)." WHERE question_id = :question_id AND type=:type";
        $params = array(':question_id'=>$question_id,'type'=>$type);
    ...
    public function getOpenid($question_id,$openid,$type='question'){
        $item = pdo_get($this->table,array('question_id'=>$question_id,'openid'=>$openid,'type'=>$type));
        return $item;
    }
    ...
    public function install(){
        ...
        if(!pdo_fieldexists($this->table,'type')){
            pdo_query("ALTER TABLE ".tablename($this->table)." ADD COLUMN `type` varchar(64) DEFAULT 'question'");
        }
