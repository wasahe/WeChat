...
class member
{
    ...
    public function getList($page,$where="",$params,$psize=10,$orderbyStr='create_time DESC'){
        global $_W,$_GPC;
        if(empty($page)){
            $page = 1;
        }
        ...
        $sql = "SELECT * FROM ".tablename($this->table)." WHERE uniacid = :uniacid {$where} ORDER BY {$orderbyStr} limit ".(($page-1)*$psize).",".$psize;
        $result = array();
    ...
    public function update(){
        ...
        if(empty($member)){
			...
			if(empty($user['avatar'])){
				$data['avatar'] = './addons/yukiho_zvoice/assets/images/default_avatar.jpg';
			}else{
			...
			if(empty($user['avatar'])){
				$data['avatar'] = IA_ROOT.'/addons/yukiho_zvoice/assets/images/default_avatar.jpg';
			}
            if(!empty($uid)){
                $data['uid'] = intval($uid);
            }
    ...
    public function install(){
        //pdo_query("update ims_yukiho_zvoice_member set labels = ','+category_id+',' where status='3'");
        ...
        if(!pdo_fieldexists($this->table,'labels')){
            pdo_query("ALTER TABLE ".tablename($this->table)." ADD COLUMN `labels` varchar(255) DEFAULT ''");
        }
        if(!pdo_fieldexists($this->table,'learn')){
            pdo_query("ALTER TABLE ".tablename($this->table)." ADD COLUMN `learn` int(11) DEFAULT '0'");
        }
    }
}