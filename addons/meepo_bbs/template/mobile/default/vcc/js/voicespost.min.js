    ...
    v.uploadrecord = function(locrecid) {
        ...
        wx.uploadVoice({
            ...
            success: function(res) {
                ...
                core.post('upload',{type:'voice',serverId:res.serverId,recTime:recTime,voicetype:voicetype,recordText:$("#hidrecordText").val()},function(data){
                	layer.close(layer_wait_win);
                    if (data.code == 1) {
                        $("#hidrecordId").val(data.msg);
                        var vhtml = $("#VoicePostOne_Tmp").html().replace("LOCALID", locrecid).replace("TIME", recTime + '"').replace("MP3PATH", "").replace("TYPE", "VOICE");
                        $("#reclist").html(vhtml);
                        addStorage();
                        layer.closeAll()
                    } else {
...
$(document).ready(function() {
    $(document).on("touchstart", "#voice-start-button", function(e) {
        e.preventDefault();
        vzan.startrecord()
    }).on("touchend touchcancel", "#voice-start-button", function(e) {
        e.preventDefault();
        vzan.stoprecord()
    }).on("click", "#voice-start-button-click", function(e) {
        e.preventDefault();
    ...
    $(document).on("click", "#voice-record-button", function(e) {
        e.stopPropagation();
        vzan.showPlayUI(0)
    });
    $(document).on("click", "#voice-complete-button", function(e) {
        e.stopPropagation();
        vzan.completevoic()
    });
    $(document).on("click", "#voice-record-button", function(e) {
    ...
    $(document).on("click", ".icon-post-voice", function(e) {
