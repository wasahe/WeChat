...
this.JpegMeta.JpegFile = function(a, b) {
    ...
    for (k += 2; k < this._binary_data.length && (d = this._binary_data.charCodeAt(k++),
    e = this._binary_data.charCodeAt(k++),
    l = k,
    d == this._DELIM) && e !== c; ) {
        for (h = JpegMeta.parseNum(">", this._binary_data, k, 2),
        k += h; k < this._binary_data.length; )
            if (d = this._binary_data.charCodeAt(k++),
            d == this._DELIM && (f = this._binary_data.charCodeAt(k++),
            0 != f)) {
                k -= 2;
                break
            }
        g = k - l,
        this._markers[e] ? (i = this._markers[e][0],
        j = this._markers[e][1]) : (i = "UNKN",
        j = void 0),
        j && this[j](e, l + 2)
    }
    if (void 0 === this.general)
    ...
    for (var t = 0; s > t; t++)
        ...
        void 0 !== this._types[k]) {
            ...
            "UNDEFINED" == j)
                ...
                for (var u = 0; m > u; u++,
                n += l)
                    ("BYTE" == j || "SHORT" == j || "LONG" == j) && o.push(JpegMeta.parseNum(a, b, n, l)),
                    ("SBYTE" == j || "SSHORT" == j || "SLONG" == j) && o.push(JpegMeta.parseSnum(a, b, n, l)),
                    "RATIONAL" == j && (p = JpegMeta.parseNum(a, b, n, 4),
                    q = JpegMeta.parseNum(a, b, n + 4, 4),
                    o.push(new JpegMeta.Rational(p,q))),
                    "SRATIONAL" == j && (p = JpegMeta.parseSnum(a, b, n, 4),
...
JpegMeta.JpegFile.prototype._exifHandler = function(a, b) {
    ...
    if (d = JpegMeta.parseNum(c, this._binary_data, b + 2, 2),
    42 !== d)
        throw new Error("Malformed TIFF meta-data. Bad magic: " + d);
    if (e = JpegMeta.parseNum(c, this._binary_data, b + 4, 4),
    this._parseIfd(c, this._binary_data, b, e, this._tifftags, "tiff", "TIFF"),
    this.tiff.ExifIfdPointer && this._parseIfd(c, this._binary_data, b, this.tiff.ExifIfdPointer.value, this._exiftags, "exif", "Exif"),
    this.tiff.GPSInfoIfdPointer) {
        if (this._parseIfd(c, this._binary_data, b, this.tiff.GPSInfoIfdPointer.value, this._gpstags, "gps", "GPS"),
...
function(a, b) {
    ...
    function b(a, b) {
        ...
        window.Worker ? Q.monitor(419377) : Q.monitor(419378);
        for (var h in g)
            g.hasOwnProperty(h) && e.append("type", g[h]);
        e.append("base64_code", c),
        f.error && d.addEventListener("error", f.error),
        f.abort && d.addEventListener("abort", f.abort),
        f.complete && d.addEventListener("load", function(b) {
    ...
    function e(a, b, c) {
        var d, e = document.createElement("canvas"), f = e.getContext("2d"), g = 0, h = c, i = c;
        for (e.width = 1,
        e.height = c,
        f.drawImage(a, 1 - Math.ceil(Math.random() * b), 0),
        d = f.getImageData(0, 0, 1, c).data; i > g; ) {
            var j = d[4 * (i - 1) + 3];
            0 === j ? h = i : g = i,
            i = h + g >> 1
        }
    ...
    function f(a, b, c, d) {
        d = d || {},
        "square" === d.type ? (a.width = d.s,
        a.height = d.s) : d.orient >= 5 && d.orient <= 8 ? (a.width = c,
    ...
    function g(a, b) {
        ...
        var g, h, i, j = b.maxW, k = b.maxH, l = b.quality, m = a.naturalWidth, o = a.naturalHeight, p = b.type, q = b.side, r = 0, s = 0;
        n() && d(a) && (m /= 2,
        o /= 2),
        "square" === p ? m > q && m > o ? (g = m * q / o,
        h = q,
        r = (m - o) / 2,
        i = q) : o > q && o > m ? (g = q,
        h = o * q / m,
        ...
        i = q) : (g = m,
        h = o,
        i = Math.min(g, h)) : m > j && m / o >= j / k ? (g = j,
        h = o * j / m) : o > k && o / m >= k / j ? (g = m * k / o,
        h = k) : (g = m,
        h = o);
        var t, u = document.createElement("canvas"), v = u.getContext("2d");
        if (f(u, g, h, $.extend({
            s: i
        }, b)),
        c(u, g, h, b.orient),
        n()) {
            var w, x, y, z, A, B, C, D, E = document.createElement("canvas"), F = E.getContext("2d"), G = 1024, H = e(a, m, o);
            for (E.width = E.height = G,
            w = 0; o > w; ) {
                for (z = w + G > o ? o - w : G,
                x = 0; m > x; )
                    y = x + G > m ? m - x : G,
            ...
            v.drawImage(a, 0, 0, m, o, 0, 0, g, h);
        if (n())
            t = u.toDataURL("image/jpeg", l);
        else {
    ...
    function h(a, b) {
        ...
        ,
        d ? c.src = a.data : c.src = webkitURL.createObjectURL(a)
    }
    function i(a, b) {
    ...
    function m(c, d) {
        ...
        h(c, function(b, h, k) {
            ...
            void setTimeout(function() {
                p ? (k.base64 = g(h, $.extend(k, d)),
                k.type = "image/other",
                e(k)) : m ? i(c, function(b) {
                    6 === b && (k = {
                        w: k.h,
                        h: k.w
                    }),
                    ...
                    0 === a.split(",")[1].indexOf("R0lG") ? (k.base64 = a,
                    k.type = "image/gif",
                    e(k)) : (k.base64 = g(h, k),
                    k.type = "image/other",
                    e(k))
