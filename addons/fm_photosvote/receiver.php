...
class Fm_photosvoteModuleReceiver extends WeModuleReceiver {
	public function receive() {
		...
		//file_put_contents(IA_ROOT.'/addons/fm_photosvote/fm_test.txt','cs',file_append);
		if ($event == 'unsubscribe') {
			//file_put_contents(IA_ROOT.'/addons/fm_photosvote/fm_test.txt', '1\n',file_append);
			/**$record = array(
						...
						if(TIMESTAMP >= $rbasic['start_time'] && TIMESTAMP <= $rbasic['end_time']){
							...
							if (!empty($vote_times)) {
								foreach ($vote_times as $key => $value) {
									...
									//file_put_contents(IA_ROOT.'/addons/fm_photosvote/fm_test.txt', '2\n');
