	...
	$.contextMenu({
		...
		callback : function(key, options) {
			...
			if (key == 'prev') {
				...
				var first = $('#nb_poster .drag:first');
				if (first.length > 0) {
...
$('#posterbg').find('button:first').click(function(){
    ...
    bscounter = setInterval(function(){
         ...
         if(oldbg!=bg){
        	 ...
        	 $('#nb_poster').css('width',img.width/2+'px').css('height',img.height/2+'px');
