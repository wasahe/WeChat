...
class Mon_EggModuleSite extends WeModuleSite
{
	...
	public function doMobileAjaxSmashing() {
       ...
       //	createRecord($uid, $egid, $pid, $status, $openid,$pname)
		if ($userPrizeCount >= $egg ['prize_limit']) {  //已超过中奖次数
			$this->createRecord($uid, $egid, 0, self::$STATUS_UNKNOW, $openid, '', null);
			$res['code'] = 200;
			$recordCount = $this->getLeftCountAndShare($egg, $openid);
			$res['left'] = $recordCount[0];
			$res['shareTip'] = $recordCount[1];
		...
		if ($pIndex == count($prizes)) { //没有中奖
			...
			if ($przie_count >= $prize['pcount'] ) { //超过数量了
				...
				$this->createRecord($uid, $egid, $prize['id'], self::$STATUS_ZJ, $openid,$prize['pname'], $prize);
				$res['code'] = 250;
				$recordCount = $this->getLeftCountAndShare($egg, $openid);
				$res['left'] = $recordCount[0];
				$res['shareTip'] = $recordCount[1];
	...
	public function createRecord($uid, $egid, $pid, $status, $openid,$pname, $prize)
	{
		$recordData = array(
			'egid' => $egid,
			'pid' => $pid,
		...
		//开启积分自动对焦
		if ($egg['jf_auto_exchange_enable'] == 1 && !empty($prize) && $prize['ptype'] == 2) {
			load()->model('mc');
			$joinUid = mc_openid2uid($openid);
			$result = mc_credit_update($joinUid, 'credit1', $prize['jf'], array($joinUid,'炫酷砸金蛋奖励积分'));
			DBUtil::updateById(DBUtil::$TABLE_EGG_RECORD,array("status"=> self::$STATUS_DH,'dhtime'=>TIMESTAMP), $rid);
		}
