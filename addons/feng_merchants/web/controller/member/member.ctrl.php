<?php
/**
 * [weliam] Copyright (c) 2014 012wz.com
 * 会员
 */
defined('IN_IA') or exit('Access Denied');
$ops = array('setting','display','modal','credit_record','ajax');
$op_names = array('会员设置列表','会员列表','后台更新积分余额','更新记录','会员设置');
foreach($ops as$key=>$value){
	permissions('do', 'ac', 'op', 'member', 'member', $ops[$key], '会员', '会员管理', $op_names[$key]);
}
...
if ($op == 'ajax') {
	if(empty($setting)){
		...
		$status = $setting['credit_type']==1?2:1;
