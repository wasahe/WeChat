...
class Feng_merchantsModuleProcessor extends WeModuleProcessor {
	public function respond() {
		...
		if ($msgtype == 'text' || $event == 'click') {
			$saler = pdo_fetch('select * from ' . tablename('tg_saler') . ' where openid=:openid and status=:status and merchantid=:merchantid', array(':status' => 1, ':openid' => $openid,':merchantid'=>$merchant['id']));
			if (empty($saler)) {
