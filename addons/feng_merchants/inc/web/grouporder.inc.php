...
if (empty($op)) {
	...
	$psize = 10;
	/*搜索条件*/
	$allgoods = pdo_fetchall("select gname from".tablename('tg_goods')."where uniacid=:uniacid and isshow=:isshow and merchantid=:merchantid",array(':uniacid'=>$_W['uniacid'],':isshow'=>1,':merchantid'=>$merchant['id']));
	$condition = "uniacid = {$_W['uniacid']}  and merchantid = '{$merchant['id']}' ";
	$time = $_GPC['time'];
	...
	foreach ($alltuan as $key => $value) {
		$refund_orders = pdo_fetchall("select * from" . tablename('tg_order') . "where tuan_id='{$value['groupnumber']}' and merchantid = '{$merchant['id']}'  and uniacid='{$_W['uniacid']}' and status=7");
		$send_orders = pdo_fetchall("select * from" . tablename('tg_order') . "where tuan_id='{$value['groupnumber']}' and uniacid='{$_W['uniacid']}' and merchantid = '{$merchant['id']}'  and status in(3,4)");
	...
	$orders = pdo_fetchall("SELECT * FROM " . tablename('tg_order') . " WHERE tuan_id = '{$groupnumber}' and merchantid = '{$merchant['id']}'  and uniacid='{$_W['uniacid']}' ORDER BY id asc");
	$goods = pdo_fetch("select * from" . tablename('tg_goods') . "where id='{$thistuan['goodsid']}'");
	if ($_GPC['type']=='submit') {
		if ($_GPC['thestatus']=='') {
			message('请选择订单状态！');
		} else {
			pdo_update('tg_group', array('groupstatus'=>$_GPC['thestatus']), array('groupnumber' => $groupnumber));
			if($_GPC['thestatus']==1){
				pdo_update('tg_order', array('status'=>6), array('tuan_id' => $groupnumber,'status'=>1));
				pdo_update('tg_order', array('status'=>6), array('tuan_id' => $groupnumber,'status'=>2));
				$num = pdo_delete('tg_order',array('tuan_id'=>$groupnumber,'mobile'=>'虚拟'));
				pdo_update('tg_group', array('lacknum' => $num), array('groupnumber' => $thistuan['groupnumber']));
			}
			if($_GPC['thestatus']==2){
				pdo_update('tg_order', array('status'=>2), array('tuan_id' => $groupnumber,'status'=>1));
				pdo_update('tg_order', array('status'=>2), array('tuan_id' => $groupnumber,'status'=>6));
				
				include_once '../addons/feng_fightgroups/source/get_head_imgs.php';
				include_once '../addons/feng_fightgroups/source/get_nickname.php';
				$filename = "../addons/feng_fightgroups/source/nickname.text";
				$url='../addons/feng_fightgroups/static/head_imgs'; //放图片的文件夹路径名称
				$getheadimgs = new get_head_imgs();
				$getnickname = new get_nicknames();
				//虚拟订单
				$t = time();
				$init = $orders[0]['createtime'];
				$num = array();
				$lacknum = $thistuan['lacknum'];
				$lack = $thistuan['lacknum'];
				$head_imgs_array = $getheadimgs->get_head_img($url, $lack);
				$nickname_array = $getnickname->get_nickname($filename,$lack);
				$time_array = $getnickname->get_randtime($init,$t,$lack);
				for ($i = 0; $i < $lacknum; $i++) {
						$lack = $lack - 1;
						$data = array(
						 'uniacid' => $_W['uniacid'],
	...
	//指定团的id
	$thistuan = pdo_fetch("select * from" . tablename('tg_group') . "where groupnumber = '{$groupnumber}' and uniacid='{$_W['uniacid']}'");
	$orders2 = pdo_fetchall("SELECT * FROM " . tablename('tg_order') . " WHERE tuan_id = '{$groupnumber}' and merchantid = '{$merchant['id']}'  and uniacid='{$_W['uniacid']}'");
	$goods = pdo_fetch("select * from" . tablename('tg_goods') . "where id='{$thistuan['goodsid']}'");
	//虚拟订单
	$t = time();
	...
	if ($nowthistuan['lacknum'] == 0) {
		...
		$orders3 = pdo_fetchall("SELECT * FROM " . tablename('tg_order') . " WHERE tuan_id = '{$groupnumber}' and merchantid = '{$merchant['id']}'  and uniacid='{$_W['uniacid']}' and status=1 and mobile<>'虚拟' ");
		foreach ($orders3 as $key => $value) {
			pdo_update('tg_order', array('status' => 2), array('id' => $value['id']));
		}

	}
	$orders = pdo_fetchall("SELECT * FROM " . tablename('tg_order') . " WHERE tuan_id = '{$groupnumber}' and merchantid = '{$merchant['id']}'  and uniacid='{$_W['uniacid']}'");
	$thistuan = pdo_fetch("select * from" . tablename('tg_group') . "where groupnumber = '{$groupnumber}'  and merchantid = '{$merchant['id']}' and uniacid='{$_W['uniacid']}'");
} elseif ($op == 'refundgroup') {
	$groupnumber = intval($_GPC['groupnumber']);
	//指定团的id
	$orders = pdo_fetchall("SELECT * FROM " . tablename('tg_order') . " WHERE tuan_id = '{$groupnumber}' and merchantid = '{$merchant['id']}'  and uniacid='{$_W['uniacid']}' and status in(1,2,6,7) and mobile<>'虚拟' ");
	$success_num=0;
	$fail_num=0;
//	echo "<pre>";print_r($orders);exit;
	foreach ($orders as $k => $value) {
		if($value['is_tuan']==3){
			$refund_record = pdo_fetch("select refundfee from".tablename('tg_refund_record')."where transid='{$value['transid']}' and uniacid='{$_W['uniacid']}' ");
	...
	$con = "uniacid = {$_W['uniacid']}  and merchantid = '{$merchant['id']}' ";
	if (!empty($groupstatus)) {
	...
	foreach ($groups as $k => $v) {
		$html .= "\n";
		$orders = pdo_fetchall("select * from" . tablename('tg_order') . "where tuan_id='{$v['groupnumber']}' and uniacid='{$_W['uniacid']}'  and merchantid = '{$merchant['id']}' ");
		if ($v['groupstatus'] == 1) {
			$tuanstatus = '团购失败';
		}
		if ($v['groupstatus'] == 2) {
