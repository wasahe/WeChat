...
if($operation=='limit'){
		...
		if (checksubmit('submitlimit')) {
			...
			load()->func('file');
			$file_path = IA_ROOT . "/addons/feng_fightgroups/arealimit/".$_W['uniacid'];
			if(!file_exists($file_path)){
				$r = mkdirs(IA_ROOT . "/addons/feng_fightgroups/arealimit/".$_W['uniacid']);
				if($r){
//					$re = @fopen(IA_ROOT . "/addons/feng_fightgroups/arealimit/".$_W['uniacid'].'/limitarea.xml',"x");
					$ret = file_put_contents(IA_ROOT . "/addons/feng_fightgroups/arealimit/".$_W['uniacid'].'/limitarea.xml', '<?xml ?>');				
				}
...
//				$ret = file_put_contents($file_path.'/arealimit.txt', "xx");
		...
		if($yunfeiop=='display'){
			$list = pdo_fetchall("SELECT * FROM " . tablename('tg_dispatch') . " WHERE uniacid = '{$_W['uniacid']}' and merchantid='{$merchant['id']}'  ORDER BY id asc");
		}elseif($yunfeiop=='post'){
			$id = $_GPC['id'];
			$dispatch = pdo_fetch("SELECT * FROM " . tablename('tg_dispatch') . " WHERE uniacid = '{$_W['uniacid']}' and id = '{$id}'");
			if (!empty($dispatch)) {
			...
			if (checksubmit('submit')) {
				...
				$thisname = pdo_fetch("select * from".tablename('tg_dispatch')."where dispatchname=:dispatchname",array(':dispatchname'=>$_GPC['dispatchname'])); 
				if(!empty($thisname)&& empty($id)){
					message("该配送方式名称被占用，请重新命名！",referer(),'error');exit;
				}
				$data = array('merchantid'=>$merchant['id'],'uniacid' => $_W['uniacid'], 'displayorder' => intval($_GPC['displayorder']), 'dispatchtype' => intval($_GPC['dispatchtype']), 'dispatchname' => trim($_GPC['dispatchname']), 'express' => trim($_GPC['express']), 'firstprice' => trim($_GPC['default_firstprice']), 'firstweight' => trim($_GPC['default_firstweight']), 'secondprice' => trim($_GPC['default_secondprice']), 'secondweight' => trim($_GPC['default_secondweight']), 'areas' => iserializer($areas), 'carriers' => iserializer($carriers), 'enabled' => intval($_GPC['enabled']));
				if (!empty($id)) {
			...
			$id = intval($_GPC['id']);
			$dispatch = pdo_fetch("SELECT id,dispatchname FROM " . tablename('tg_dispatch') . " WHERE id = '$id' AND uniacid=" . $_W['uniacid'] . "");
			if (empty($dispatch)) {
