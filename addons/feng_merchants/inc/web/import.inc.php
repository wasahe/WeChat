...
if ($operation == 'display') {
	...
	$total_tuan = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and merchantid='{$merchant['id']}'  and status=2 and is_tuan in(1,2,3,4) and is_hexiao=0", $paras);
	$total_single = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and merchantid='{$merchant['id']}'  and status=2 and is_tuan=0 and is_hexiao=0", $paras);
	$total_all = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and merchantid='{$merchant['id']}'  and status=2 and is_hexiao=0");
		...
		if (is_uploaded_file($uploadfile)) {
			...
			if ($ftype == 'xls') {
				...
				for ($j = 2; $j <= $highestRow; $j++) {
					...
					$expressOrder = trim($objPHPExcel -> getActiveSheet() -> getCell("K$j") -> getValue());
					$expressName = trim($objPHPExcel -> getActiveSheet() -> getCell("L$j") -> getValue());
					if (!empty($expressOrder) && !empty($expressName)) {
						...
						if($res){
								$m=$this->updateaccount($order['price'],$order['merchantid']);
						}
						if ($res) {
							$succ_result += 1;
						} else {
						...
						$res = $sendmessage -> send_success($orderNo, $order['openid'], $expressName, $expressOrder, $set, $url1, $url2);
						/*发货提醒模板消息*/
					} else {
						if (!empty($orderNo)) {
							$error_result += 1;
			    ...
			    for ($i = 1; $i < $len_result; $i++) { //循环获取各字段值 
			        ...
			        $expressOrder = trim(iconv('gb2312', 'utf-8', $result[$i][10])); 
			        $expressName =trim(iconv('gb2312', 'utf-8', $result[$i][11]));
...
//	$orders = pdo_fetchall("select * from" . tablename('tg_order') . "where $condition and status=2");
	$orders = pdo_fetchall("select DISTINCT address from" . tablename('tg_order') . "where $condition and status=2 and is_hexiao=0");
	if ($status == '0') {
		$str = '单独购买待发货订单_' . time();
	}
	if ($status == '1') {
	...
	$html = "\xEF\xBB\xBF";
	/* 输出表头 */
	$filter = array('aa' => '订单编号','ll'=>'团ID', 'bb' => '姓名', 'cc' => '电话','dd' => '总价(元)', 'ee' => '状态', 'ff' => '下单时间', 'gg' => '商品名称', 'hh' => '收货地址', 'ii' => '微信订单号', 'jj' => '快递单号', 'kk' => '快递名称','mm'=>'买家留言','pp'=>'商品规格','oo'=>'数量' );
	foreach ($filter as $key => $title) {
		$html .= $title . "\t,";
	}
	$html .= "\n";
	foreach ($orders as $k => $vv) {
		$order_same = pdo_fetchall("select * from" . tablename('tg_order') . "where address='{$vv['address']}' and merchantid='{$merchant['id']}'  and status=2 and $condition and is_hexiao=0");
