...
if (checksubmit('submit')) {
	...
	if( isset($_FILES['submit_data']) ){
        ...
        function emptyDistrict($_sdata){
            if( empty($_sdata['province']) || empty($_sdata['city']) || empty($_sdata['district']) ){
                return true;
            } else {
                return false;
            }
	...
	foreach ($attrs as $row) {
		if( $row['is_must'] && ( empty($submit_data[$row['attr_id']]) || $row['type']=='district' && emptyDistrict($submit_data[$row['attr_id']]) ) ){
	...
	foreach ( $submit_data as $attr_id => $arr ) {
		$insert_data = array(
			...
			'attr_id'	=>	$attr_id,
                ...
                if( is_array($arr) && isset($arr['province']) ){
		...
		if( is_array($arr) ){
			...
			$insert_data['attr_value'] = emoji_unified_to_html(trim($arr));
			pdo_insert('fwei_forms_values', $insert_data);
		}
	}
