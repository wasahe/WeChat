...
class Meepo_bigerwallModuleSite extends WeModuleSite{
    ...
    public function doWebCheckvote(){
        ...
        if ($operation == 'display'){
            ...
            $pindex = max(1, intval($_GPC['page']));
            $psize = 20;
            $list = pdo_fetchall("SELECT * FROM " . tablename('weixin_vote') . " WHERE weid=:weid AND rid = :rid ORDER BY res DESC LIMIT " . ($pindex - 1) * $psize . ",{$psize}", array(':weid' => $weid, ':rid' => $id));
            if(is_array($list)){
                foreach($list as & $row){
                    $ridname = pdo_fetchcolumn("SELECT name FROM " . tablename('rule') . " WHERE id='{$row['rid']}' AND uniacid='{$weid}'");
                    $row['ridname'] = $ridname;
                }
                unset($row);
            }
            ...
            if (checksubmit('submit')){
                ...
                if (!empty($voteid)){
                    ...
                    $voteid = pdo_insertid();
                    message('新增投票成功！', $this -> createWebUrl('Checkvote', array('id' => $id, 'op' => 'post')), 'success');
                }
            }
        ...
        if ($operation == 'display'){
            ...
            if(is_array($list)){
                foreach($list as & $row){
                    ...
                    $ridname = pdo_fetchcolumn("SELECT name FROM " . tablename('rule') . " WHERE id='{$row['luckid']}' AND uniacid='{$weid}'");
            ...
            if (checksubmit('submit')){
                ...
                if(!empty($_GPC['nd'])){
                    ...
                    if(strpos($ndone, '，') || strpos($ndone, ' ')){
                        ...
                        $signtotalall = pdo_fetchall('SELECT	`id` FROM ' . tablename('weixin_flag') . " WHERE weid = '{$weid}' AND rid='{$id}'");
            ...
            message('操作成功！', $this -> createWebUrl('awardmanage', array('id' => $id, 'op' => 'post', 'cjid' => $cjid)), 'success');
        }else{
            message('请求方式不存在');
        }
