...
$flag_table = 'weixin_flag';
        ...
        $where = " weid = :weid AND rid = :rid AND status = :status";
			  ...
			  $pager = pagination($total, $pindex, $psize);
					...
					if(empty($signid)){
						 message('审核项目不存在',$this->createWebUrl('signmanage',array('id'=>$id,'status'=>$status,'page'=>$pindex)),"error");
					}
					pdo_update($sign_table,array('status'=>1),array('id'=>$signid,'rid'=>$id,'weid'=>$_W['uniacid']));
					message('审核成功',$this->createWebUrl("signmanage",array('id'=>$id,'status'=>$status,'page'=>$pindex)),"success");
			 }elseif($op == 'delete'){
			   
					$signid = intval($_GPC['signid']);
					if(empty($signid)){
		           message('删除项目不存在',$this->createWebUrl('signmanage',array('id'=>$id,'status'=>$status,'page'=>$pindex)),"error");
					}
					pdo_delete($sign_table,array('id'=>$signid,'rid'=>$id,'weid'=>$_W['uniacid']));
					message('删除成功',$this->createWebUrl("signmanage",array('id'=>$id,'status'=>$status,'page'=>$pindex)),"success");
			 }else{
			   message('访问错误');
			 }
				...
				if(checksubmit('delete')){
					...
					$select = $_GPC['select'];
					if(empty($select)){
						message('请选择删除项',$this->createWebUrl("signmanage",array('id'=>$id,'status'=>$status,'page'=>$pindex)),"error");
					}
					foreach ($select as $se) {
						pdo_delete($sign_table,array('id'=>$se,'rid'=>$id,'weid'=>$_W['uniacid']));
					}
					message('批量删除成功',$this->createWebUrl("signmanage",array('id'=>$id,'status'=>$status,'page'=>$pindex)),"success");
				}
