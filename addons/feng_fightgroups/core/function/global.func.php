...
function web_url($segment, $params = array()) {
	...
	if((!empty($_GPC['merchantid']) || $params['role']=='merchant')&& $_SESSION['role']=='merchant'){
		$url = $_W['siteroot'] . 'addons/feng_fightgroups/web/merch.php?';
	}else{
...
//	Util::wl_log('weixin_notify_core_siteroot',TG_DATA, $_W['siteroot']); //写入异步日志记录
	if (!empty($do)) {
		$url .= "do={$do}&";
	}
	if (!empty($ac)) {
...
function permissions($do, $ac, $op, $v1, $v2, $v3, $n1, $n2, $n3) {
	global $_W;
	$role_do = pdo_fetch("select * from" . tablename('tg_user_node') . "where do='{$v1}'");
	$role_ac = pdo_fetch("select * from" . tablename('tg_user_node') . "where do='{$v1}' and ac='{$v2}'");
...
function createUniontid(){
	$moduleid = pdo_fetchcolumn("SELECT mid FROM ".tablename('modules')." WHERE name = :name", array(':name' =>'feng_fightgroups'));
	$moduleid = empty($moduleid) ? '000000' : sprintf("%06d", $moduleid); 
	$uniontid = date('YmdHis').$moduleid.random(8,1);//生成商户订单号
	return $uniontid;
}
