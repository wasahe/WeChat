...
//		$orderInfo = Util::getDataByCacheFirst('order',$id,array('Util','getSingelData'),array($select,'tg_order',$where));
	...
	static function initSingleOrder($orderInfo){
		...
		if($goodsInfo){
			...
			$orderstatus = array('0' => array('css' => 'default', 'name' => '待付款'), '1' => array('css' => 'info', 'name' => '已付款'), '2' => array('css' => 'warning', 'name' => '待消费'), '3' => array('css' => 'success', 'name' => '待消费'), '4' => array('css' => 'success', 'name' => '已消费'), '5' => array('css' => 'default', 'name' => '已取消'), '6' => array('css' => 'danger', 'name' => '待退款'), '7' => array('css' => 'default', 'name' => '已退款'));
	...
	*/  
	static function refundMoney($id,$money,$refund_reason,$type=5){
		...
		if($orderinfo['pay_type'] == '1'){ //余额支付
			...
			if(!empty($data['refund_id']) && $data['return_code']=='SUCCESS') $refund = TRUE;
		}
		...
		if($refund){
			...
			Util::deleteCache('goods', $orderinfo['g_id']);
	...
	*/  
	static function getNumRefundOrder($num){
		global $_W;
		if($num==0){
			return pdo_fetchall("select price,status,transid,pay_type,orderno,id from".tablename("tg_order")."where uniacid={$_W['uniacid']} and status=6");
		}else{
			return pdo_fetchall("select price,status,transid,pay_type,orderno,id from".tablename("tg_order")."where uniacid={$_W['uniacid']} and status=6 ORDER BY ptime DESC " . "LIMIT 0,". $num);
		}
	}
