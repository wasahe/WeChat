...
class message{
	...
	static function activity_tip($content,$openid, $name, $url) {
	    global $_W;
	...
	static function activity_lottery($openid, $level,$a_name,$p_name, $url) {
	    global $_W;
	    wl_load()->model('setting');
	    $setting = setting_get_by_name("message");
	    $m_activity = $setting['m_activity_lottery'];
	...
	static function activity_result($openid, $a_name,$p_name, $url) {
	    global $_W;
	    wl_load()->model('setting');
	    $setting = setting_get_by_name("message");
	...
	static function daipay_success($openid, $price, $name, $orderno, $goodsname, $time, $message, $url) {
	    global $_W;
	    wl_load()->model('setting');
	    $setting = setting_get_by_name("message");
	...
	static function pay_success($openid, $price, $goodsname, $url) {
	    global $_W;
	    wl_load()->model('setting');
	    $setting = setting_get_by_name("message");
	    $m_pay = $setting['m_pay'];
	...
	static function group_success($tuan_id, $url) {
	    global $_W;
	    wl_load()->model('setting');
	    $setting = setting_get_by_name("message");
	    $m_tuan = $setting['m_tuan'];
	    $alltuan = pdo_fetchall("select openid,lotteryid from" . tablename('tg_order') . "where tuan_id = '{$tuan_id}' and status in(1,2,3,4) and mobile<>'虚拟'");
	    $tuan_first_order = pdo_fetch("select openid,g_id,tuan_id,lotteryid from" . tablename('tg_order') . "where tuan_first=1 and tuan_id='{$tuan_id}'");
	    ...
	    if($tuan_first_order['lotteryid']>=1){
	    	$content = "恭喜抽奖团组团成功,您编号为【" . $tuan_first_order['tuan_id'] . "】的团组团成功,请及时关注抽奖动态.";
	    }else{
	    	$content = "恭喜组团成功,您编号为【" . $tuan_first_order['tuan_id'] . "】的团组团成功,请及时关注发货动态.";
	    }
	    $postdata = array(
	        "first" => array(
	            "value" => $content,
	...
	static function send_success($orderno, $openid, $express, $expressn, $url) {
	    global $_W;
	    wl_load()->model('setting');
	    $setting = setting_get_by_name("message");
	...
	static function refund_success($orderno, $openid, $price, $url,$refund_reason='组团失败,系统已将您的支付金额全数退还.') {
	    global $_W;
	    wl_load()->model('setting');
	    $setting = setting_get_by_name("message");
	    $m_ref = $setting['m_ref'];
		if(empty($refund_reason)){
			$refund_reason = '组团失败,系统已将您的支付金额全数退还.';
		}
	    $postdata = array(
	        "first" => array(
	...
	static function cancelorder($openid, $price, $goodsname, $orderid, $url) {
	    global $_W;
	    wl_load()->model('setting');
	    $setting = setting_get_by_name("message");
