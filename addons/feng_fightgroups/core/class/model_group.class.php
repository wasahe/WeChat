	...
	*/ 
	static function updateGourpStatus() {
		...
		foreach ($allgroups as $key => $value) {
			if ($value['endtime'] < $now && $value['lacknum'] > 0) {
				...
				Util::deleteCache('group', $value['groupnumber']);
	...
	static function setLotteryOver(){
		...
		foreach($lottery as $item){
			...
			Util::deleteCache('goods', $item['fk_goodsid']);
		}
	}
	/*获取所有抽奖活动ID*/
	...
	/*抽奖*/
	static function doLottery($lottery_id){
		global $_W;	
		$lottery = pdo_fetch("select id,prize,num,num2,num3,gname from".tablename("tg_lottery")."where id={$lottery_id}  and uniacid={$_W['uniacid']}");
		$prize = unserialize($lottery['prize']);
		$ordersNum = self::getLotteryOrdersNum($lottery_id);
		
		//内定1,2,3等奖
		$default1 =  pdo_fetchcolumn("select count(id) from".tablename('tg_order')."where lotteryid={$lottery_id} and uniacid={$_W['uniacid']} and lottery_status = 5 ") ;
		$default2 =  pdo_fetchcolumn("select count(id) from".tablename('tg_order')."where lotteryid={$lottery_id} and uniacid={$_W['uniacid']} and lottery_status = 6 ") ;
		$default3 =  pdo_fetchcolumn("select count(id) from".tablename('tg_order')."where lotteryid={$lottery_id} and uniacid={$_W['uniacid']} and lottery_status = 7 ") ;
				
		$num  = $lottery['num'] - $default1;
		$num2 = $lottery['num2'] - $default2;
		$num3 = $lottery['num3'] - $default3;
		if($num<0)return false;
		$sql = "SELECT orderno,openid,id,tuan_id FROM ".tablename('tg_order')."WHERE lotteryid={$lottery_id} and status=2 and uniacid={$_W['uniacid']} and lottery_status not in(2,3,4,5,6,7)";
		...
		foreach($firsterOrder as $first){ //一等奖
			if(pdo_update("tg_order",array('lottery_status'=>2),array('orderno'=>$first['orderno']))){
				Util::deleteCache('order', $first['id']);
				$url = app_url("order/group",array('tuan_id'=>$first['tuan_id']));
				$content = "恭喜获得一等奖【".$lottery['gname']."】";
				message::activity_tip($content, $first['openid'], $lottery['gname'], $url);
			}
		}
		...
		if($lottery['num2'] == -1){ //除了一等奖全是二等奖
			...
			$sql = "SELECT orderno,id FROM ".tablename('tg_order')."WHERE lotteryid={$lottery_id} and status=2 and uniacid={$_W['uniacid']} and lottery_status not in(2,3,4,5,6,7)";
			...
			foreach($seconderOrder as $second){
				if(pdo_update("tg_order",array('lottery_status'=>3),array('orderno'=>$second['orderno']))){
					if ($prize['first']['radio']==1 || $prize['first']['radio']==3){
						 ...
						 Util::deleteCache('order', $second['id']);
					}
				}
			}
				...
				foreach($thirderOrder as $third){
					if(pdo_update("tg_order",array('lottery_status'=>4),array('orderno'=>$third['orderno']))){
						if ($prize['second']['radio']==1 || $prize['second']['radio']==3){
							...
							Util::deleteCache('order', $third['id']);
						}
