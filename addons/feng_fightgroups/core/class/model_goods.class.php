...
//		$goodsInfo = Util::getNumData($select,"tg_goods", $where,$order,$pindex, $psize,$ifpage);
	...
	static function initSingleGoods($goodsInfo){
		...
		if($goodsInfo['id'])
		$params = pdo_fetchall("SELECT * FROM" . tablename('tg_goods_param') .  "WHERE goodsid = {$goodsInfo['id']} limit 0,3 ");
	...
	* @return array 
	*/  
	static function getFreight($id,$addrid,$openid,$goods){
		$freight=0;
		if($addrid)
		...
		if(($adress_fee['province'] == '' || $adress_fee['city'] == '' || $adress_fee['county'] == '') && $goods['is_hexiao']!=2  ){
			$adress_fee = '';
		}
