...
if ($op == 'display') {
	...
	//$resp = ihttp_request('http://weixin.weliam.cn/addons/weliam_manage/api.php', array('type' => 'user','module' => 'feng_fightgroups','website' => $siteid,'domain'=> $domain),null,1);
	$resp = @json_decode($resp['content'], true);
	$ip = $resp['ip'];
	//$result = ihttp_request('http://weixin.weliam.cn/addons/weliam_manage/api.php', array('type' => 'checkauth','module' => 'feng_fightgroups'),null,5);
	$result = @json_decode($result['content'], true);
	
	if (checksubmit()) {
		$data = array('ip' => $_GPC['ip'],'domain' => $_GPC['domain'],'siteid' => $_GPC['siteid'],'code' => $_GPC['code']);
		tg_syssetting_save($data,'auth');
		//$resp = ihttp_request('http://weixin.weliam.cn/addons/weliam_manage/api.php', array('type' => 'grant','module' => 'feng_fightgroups','code' => $data['code']),null,1);
		$resp = @json_decode($resp['content'], true);
		if($resp['errno'] == 1){
...
if ($op == 'upgrade') {
    ...
    //$resp = ihttp_post('http://weixin.weliam.cn/addons/weliam_manage/api.php', array(
        'type' => 'check',
