<?php
	...
	if($op == 'display'){
		...
		$orders = pdo_fetchall('select price,status from'.tablename('tg_order').'where merchantid='.$id.' and status in(1,2,3,4,6)');
		$km = 0;
		$zm = 0;
		foreach($orders as $ke => $va){
			if(in_array($va['status'], array(3,4)))$km += $va['price'];
			$zm += $va['price'];
		}
	...
	if($op == 'search'){ //选择通知管理员
		...
		$type = $_GPC['t'];
		if ($keyword != '') $con .= " and nickname LIKE '%{$keyword}%'";
	...
	if($op == 'edit') { //商家编辑
		...
		if(!empty($id)){
			...
			if(empty($merchant)) message('商家信息不存在', web_url('application/merchant', array('op'=>'display')), 'success');
	...
	if($op == 'account_display'){ //可结算商家列表
		...
		$psize = 15;
		$where=array();
		if($_GPC['keyword']) $where['id^name^linkman_name^linkman_mobile'] = $_GPC['keyword'];
	...
	if($op == 'data_record'){ //金额变动记录
		...
		foreach($moneyRecord as &$item){
			if($item['orderid'])$item['order'] = model_order::getSingleOrder($item['orderid'], '*');
		}
		include wl_template('application/merchant/account');
	}
	if($op == 'account') { //结算操作
		...
		if (checksubmit('submit') && $_GPC['accountType']=='weixin') {
			...
			if(is_numeric($money)){
				...
				if(MERCHANTID){
					...
					$result = model_merchant::finance($merchant['openid'], $money * 100, '商家提现');  //结算操作
					if (is_error($result)){
						message('微信钱包提现失败: ' . $result['message'], '', 'error'); // 结算失败
					}else{ //结算成功
	...
	if($op == 'merchantApply'){ //提现申请
		...
		if($_GPC['id']){
			...
			if($_GPC['type']==1){
				...
				if(!is_numeric($money))message('金额错误！', referer(), 'error');
				if($money<1) message('到账金额需要大于1元！', referer(), 'error');
				if($no_money<$money) message('您没有足够的可结算金额！', referer(), 'error');
