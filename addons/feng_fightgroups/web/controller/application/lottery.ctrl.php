...
//			wl_debug($r);
	...
	if ($todo == 'detail') {
		...
		$lottery_id = $_GPC['lottery_id'];
		$lottery = pdo_fetch("select gname,gimg,endtime,status,num,num2,num3,prize,dostatus from".tablename('tg_lottery')."where uniacid=:uniacid and id=:id  ",array(':uniacid'=>$_W['uniacid'],':id'=>$lottery_id));
		...
		if($type == 'order'){
			...
			$list = pdo_fetchall("select is_hexiao,orderno,createtime,address,addname,mobile,status,pay_price,lottery_status,id from".tablename('tg_order')."where status in(1,2,3,4,6,7) and lottery_status in(-1,1,2,3,4,5,6,7) and lotteryid={$lottery_id} $con  order by id desc " . "LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
			$total = pdo_fetchcolumn("select COUNT(id) from" . tablename('tg_order') . "where status in(1,2,3,4,6,7) and lotteryid={$lottery_id} ");
			$pager = pagination($total, $pindex, $psize);		
			if($_GPC['setone']){
				$id = $_GPC['id'];
		...
		if($type == 'process'){
			...
			$list = pdo_fetchall("select is_hexiao,orderno,createtime,address,addname,mobile,status,pay_price,lottery_status,id from".tablename('tg_order')."where status in(1,2,3,4,6,7)  and lotteryid={$lottery_id} and  {$lottery_sta}  order by id desc ");
		}
