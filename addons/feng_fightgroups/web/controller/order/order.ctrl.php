...
$op_names = array('更新订单','删除订单','订单概况','订单列表','订单详情','导出','卖家备注','修改收货地址','确认付款','发货','取消发货','退款','确认核销','取消核销');
...
if ($op == 'received') {if (empty($starttime) || empty($endtime)) {//初始化时间
	...
	$pager = $orderData[1];
	$isHexiaoStr = $_GPC['orderType']!='fetch' ?" and uniacid='{$_W['uniacid']}' and mobile<>'虚拟' and is_hexiao=0  and lottery_status in(0,2)".TG_MERCHANTID : " and uniacid='{$_W['uniacid']}' and mobile<>'虚拟' and is_hexiao in(1,2) and lottery_status in(0,2) and mobile<>'虚拟' ".TG_MERCHANTID;
...
if($op=='confirmsend'){
	...
	if($r){
		if(!empty($item['merchantid'])){
			...
			model_merchant::updateNoSettlementMoney($item['price'], $item['merchantid']);//更新可结算金额
		}
		$oplogdata = serialize($item);
...
if($op == 'output'){
	...
	$html = "\xEF\xBB\xBF";
	$filter = array('aa' => '订单编号', 'bb' => '姓名', 'cc' => '电话', 'dd' => '总价(元)', 'ee' => '状态', 'ff' => '下单时间', 'gg' => '商品名称', 'hh' => '收货地址', 'ii' => '微信订单号', 'jj' => '快递单号', 'kk' => '快递名称','ll'=>'地址类型','mm'=>'商品规格','nn'=>'购买数量','oo'=>'客户留言','pp'=>'我的备注','qq'=>'订单类型','rr'=>'核销门店','ss'=>'核销店员');
