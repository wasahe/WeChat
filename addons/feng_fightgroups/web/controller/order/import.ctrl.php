...
if ($op == 'display') {
	...
	$total_tuan = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and status=2 and is_tuan in(1,2,3,4) and is_hexiao=0 ".TG_MERCHANTID."", $paras);
	$total_single = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and status=2 and is_tuan=0 and is_hexiao=0 ".TG_MERCHANTID."", $paras);
	$total_all = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tg_order') . " WHERE uniacid='{$_W['uniacid']}' and status=2 and is_hexiao=0 ".TG_MERCHANTID."");
} elseif ($op == 'import') {
	...
	$orderData = model_order::getNumOrder('*', $where, 'createtime desc', 0, 0, 0);
	$orders = $orderData[0];
	if ($status == '0') $str = '单独购买待发货订单_' . time();
	if ($status == '1') $str = '团购成功待发货订单_' . time();
	if ($status == '') $str = '全部待发货订单' . time();
	$html = "\xEF\xBB\xBF";
	$filter = array('aa' => '订单编号', 'bb' => '姓名', 'cc' => '电话', 'dd' => '总价(元)', 'ee' => '状态', 'ff' => '下单时间', 'gg' => '商品名称', 'hh' => '收货地址', 'ii' => '微信订单号', 'jj' => '快递单号', 'kk' => '快递名称','ll'=>'团ID','pp'=>'商品规格','oo'=>'购买数量','mm'=>'买家留言');
