...
load()->func('tpl');
$ops = array('display', 'post', 'single_op', 'batch','setgoodsproperty','copy','selectgift','taobaourl','asdasd','tag','pass');
$op_names = array('商品列表','新增/修改商品','上下架/售罄/删除/彻底删除','批量设置','设置商品属性','复制商品','选择赠品','导入淘宝信息');
...
if ($op == 'post') {
	...
	if (checksubmit()) {
		$data = $_GPC['goods'];
		if($_GPC['hexiaolimittime_status'] == 'on'){
			$data['hexiaolimittime'] = strtotime($data['hexiaolimittime']);
		}else {
			$data['hexiaolimittime'] = null;
		}  
		$data['hasoption']=$_GPC['hasoption'];
		$data['allsalenum'] = $data['salenum'] + $data['falsenum'];
		if($_GPC['storeids'])$data['hexiao_id']=serialize($_GPC['storeids']);//核销ID
...
if($op == 'copy'){
	...
	$goods['isshow']=2;
	$goods['hasoption']=0;
	if(pdo_insert("tg_goods",$goods))die(json_encode(array('errno'=>0)));
