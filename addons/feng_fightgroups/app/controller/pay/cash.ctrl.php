 ...
 */
	...
	if($_GPC['creditType']=='recharge'){ //余额充值。
		$order = pdo_fetch("select orderno,num from".tablename('tg_credit1rechargerecord')."where id={$_GPC['orderid']}");
...
if ($_W['isajax']) {
	...
	if (!empty($setting['payment']['credit']['switch'])) $credtis = mc_credit_fetch($_W['member']['uid']); //如果微擎余额开启了，则获取微擎余额。
