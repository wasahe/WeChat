...
if(!empty($tuan_id)){
    ...
    $myorder = pdo_fetch("select g_id,lottery_status from".tablename('tg_order')."where openid = '{$_W['openid']}' and tuan_id = {$tuan_id} and status in(1,2,3,4,6,7) ");
  	...
  	if($tuaninfo['iflottery'] && $tuaninfo['lottery_id']>0) {
		...
		switch($myorder['lottery_status']){
	    		case 1: $myStatus = $lottery['dostatus']?'未中':'待抽奖';
	    		...
	    		case 4: $myStatus = '三等奖';$myStatus2 = '恭喜获得三等奖';break;
	    		...
	    		case 7: $myStatus = $lottery['dostatus']?'三等奖':'待抽奖';$myStatus2 = $lottery['dostatus']?'恭喜获得三等奖':'';break;
		...
		$ordersLotteryData=model_order::getNumOrder('openid,addname,mobile,addname,createtime,lottery_status', $where, 'ptime asc', $pindex, 8, 1);
	    ...
	    foreach($ordersLottery as$key =>&$value){
	    	...
	    	switch($value['lottery_status']){
	    		...
	    		case 4: $ordersLottery[$key]['lottery_status_name'] = '三等奖';break;
	    		...
	    		case 7: $ordersLottery[$key]['lottery_status_name'] = $lottery['dostatus']?'三等奖':'待抽奖';break;
				default:$ordersLottery[$key]['lottery_status_name'] = '未组团成功';break;
	    	}
		...
		if(!empty($lottery['dostatus'])){
			...
			foreach($ordersLotteryZ as$key =>&$value){
		    	...
		    	switch($value['lottery_status']){
		    		...
		    		case 4: $ordersLotteryZ[$key]['lottery_status_name'] = '三等奖';break;
		    		...
		    		case 7: $ordersLotteryZ[$key]['lottery_status_name'] = $lottery['dostatus']?'三等奖':'待抽奖';break;
	...
	$share_desc = !empty($goods['share_desc']) ? $goods['share_desc'] : $config['share']['share_desc'];
	$config['share']['share_title'] = "我参加了".$goods['gname']."拼团，快来加入吧！";
	$config['share']['share_desc'] = "【差".$tuaninfo['lacknum']."人】".$share_desc;
