window.onload = function() {
	var MainManager = {
		...
		init: function() {
			...
			FormManager.changeLihe(imgChildren[0].dataset.lihe);
		...
		changeIdentifier: function(index) {
			...
			FormManager.changeLihe(childrenLi[index].dataset.lihe);
		},
		destroy: function() {
	...
	var FormManager = {
		dom: {
			...
			btnPass: ".btn-pass",
			btnClose: ".btn-close",
			inputName: "[name='realname']",
			inputTel: "[name='mobile']",
			inputqq: "[name='qq']",
			inputemail: "[name='email']",
			inputaddress: "[name='address']",
			inputgender: "[name='gender']",
			inputtelephone: "[name='telephone']",
			inputidcard: "[name='idcard']",
			inputcompany: "[name='company']",
			inputoccupation: "[name='occupation']",
			inputposition: "[name='position']",
			inputPass: "[name='smsverify']",
			inputPrize: "[name='info-prize']",
			inputLihe: "[name='info-lihe']",
			inputSession_token: "[name='session_token']",
			container: ".container",
			tip: ".submit-tip"
		},
		canSubmit: true,
		...
		changeLihe: function(num) {
			this.dom.inputLihe.value = num;
		},
		getPass: function() {
		...
		checkForm: function(e) {
	        ...
	        $(".btn-send").css("background","#b7b7b7").siblings("input").css("background","#b7b7b7");
