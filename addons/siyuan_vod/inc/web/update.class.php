...
if (!defined('PCLZIP_TEMPORARY_FILE_RATIO')) {
	define('PCLZIP_TEMPORARY_FILE_RATIO', 0.46999999999999997);
}
...
class PclZip
{
	...
	public function privParseOptions(&$p_options_list, $p_size, &$v_result_list, $v_requested_options = false)
	{
		...
		if ($v_requested_options !== false) {
			...
			while ($key = key($v_requested_options)) {
	...
	public function privFileDescrParseAtt(&$p_file_list, &$p_filedescr, $v_options, $v_requested_options = false)
	{
		...
		foreach ($p_file_list as $v_key => $v_value ) {
			...
			return PCLZIP_ATT_FILE_CONTENT;
	...
	public function privAddFileList($p_filedescr_list, &$p_result_list, &$p_options)
	{
		...
		while (sizeof($p_result_list) && ($j < ($p_filedescr_list)) && ($v_result == 1)) {
	...
	public function privExtractByRule(&$p_file_list, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
	{
		...
		while ($i < $v_central_dir['entries']) {
			...
			if (isset($p_options[PCLZIP_OPT_BY_NAME]) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
				...
				while (($p_options[PCLZIP_OPT_BY_NAME] != 0) && ($j < ($p_options[PCLZIP_OPT_BY_NAME])) && !$v_extract) {
					if (substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1) == '/') {
						if ((strlen($p_options[PCLZIP_OPT_BY_NAME][$j]) < strlen($v_header['stored_filename'])) && (substr($v_header['stored_filename'], 0, strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
							$v_extract = true;
						}

					}
					 else if ($v_header['stored_filename'] == $p_options[PCLZIP_OPT_BY_NAME][$j]) {
						$v_extract = true;
					}
				...
				while (($p_options[PCLZIP_OPT_BY_INDEX] != 0) && ($j < ($p_options[PCLZIP_OPT_BY_INDEX])) && !$v_extract) {
					if (($p_options[PCLZIP_OPT_BY_INDEX][$j]['start'] <= $i) && ($i <= $p_options[PCLZIP_OPT_BY_INDEX][$j]['end'])) {
						$v_extract = true;
					}


					if ($p_options[PCLZIP_OPT_BY_INDEX][$j]['end'] <= $i) {
						$j_start = $j + 1;
					}
	...
	public function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
	{
		...
		if ($p_entry['status'] == 'ok') {
			if (file_exists($p_entry['filename'])) {
				if (is_dir($p_entry['filename'])) {
				 ...
				 else if ($p_entry['mtime'] < filemtime($p_entry['filename'])) {
					if (isset($p_options[PCLZIP_OPT_REPLACE_NEWER]) && ($p_options[PCLZIP_OPT_REPLACE_NEWER] === true)) {
					}
					 else {
						$p_entry['status'] = 'newer_exist';

						if (isset($p_options[PCLZIP_OPT_STOP_ON_ERROR]) && isset($p_options[PCLZIP_OPT_STOP_ON_ERROR]) && ($p_options[PCLZIP_OPT_STOP_ON_ERROR] === true)) {
				...
				if (($v_result = $this->privDirCheck($v_dir_to_check, ($p_entry['external'] & 16) == 16)) != 1) {
		...
		if ($p_entry['status'] == 'ok') {
			if (!($p_entry['external'] & 16) == 16) {
				if ($p_entry['compression'] == 0) {
				 ...
				 else if (($p_entry['flag'] & 1) == 1) {
					PclZip::privErrorLog(PCLZIP_ERR_UNSUPPORTED_ENCRYPTION, 'File \'' . $p_entry['filename'] . '\' is encrypted. Encrypted files are not supported.');
					return PclZip::errorCode();
				}
				 else if (!isset($p_options[PCLZIP_OPT_TEMP_FILE_OFF]) && (isset($p_options[PCLZIP_OPT_TEMP_FILE_ON]) || (isset($p_options[PCLZIP_OPT_TEMP_FILE_THRESHOLD]) && ($p_options[PCLZIP_OPT_TEMP_FILE_THRESHOLD] <= $p_entry['size'])))) {
		...
		if ($p_entry['status'] == 'aborted') {
		 ...
		 else if (isset($p_options[PCLZIP_CB_POST_EXTRACT])) {
			$v_local_header = array();
			$this->privConvertHeader2FileInfo($p_entry, $v_local_header);
	...
	public function privDeleteByRule(&$p_result_list, &$p_options)
	{
		...
		while ($i < $v_central_dir['entries']) {
			...
			if (isset($p_options[PCLZIP_OPT_BY_NAME]) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
				...
				while (($p_options[PCLZIP_OPT_BY_NAME] != 0) && ($j < ($p_options[PCLZIP_OPT_BY_NAME])) && !$v_found) {
					if (substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1) == '/') {
						if ((strlen($p_options[PCLZIP_OPT_BY_NAME][$j]) < strlen($v_header_list[$v_nb_extracted]['stored_filename'])) && (substr($v_header_list[$v_nb_extracted]['stored_filename'], 0, strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
							$v_found = true;
						}
						 else if ((($v_header_list[$v_nb_extracted]['external'] & 16) == 16) && (($v_header_list[$v_nb_extracted]['stored_filename'] . '/') == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
							$v_found = true;
						}

					}
					 else if ($v_header_list[$v_nb_extracted]['stored_filename'] == $p_options[PCLZIP_OPT_BY_NAME][$j]) {
						$v_found = true;
					}
				...
				while (($p_options[PCLZIP_OPT_BY_INDEX] != 0) && ($j < ($p_options[PCLZIP_OPT_BY_INDEX])) && !$v_found) {
					if (($p_options[PCLZIP_OPT_BY_INDEX][$j]['start'] <= $i) && ($i <= $p_options[PCLZIP_OPT_BY_INDEX][$j]['end'])) {
						$v_found = true;
					}
...
function PclZipUtilPathInclusion($p_dir, $p_path)
{
	...
	while (sizeof($v_list_path) && sizeof($v_list_path) && ($j < $v_list_path_size) && $v_result) {
		if ($v_list_dir[$i] == '') {
			++$i;
			continue;
		}
	...
	if ($v_result) {
		while (($v_list_path[$j] != '') && ($v_list_path[$j] != '') && ($v_list_path[$j] == '')) {
			++$j;
		}
		while (($v_list_path[$j] == '') && ($v_list_path[$j] == '') && ($v_list_dir[$i] == '')) {
			++$i;
		}
