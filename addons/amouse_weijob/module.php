...
class Amouse_weijobModule extends WeModule
{
    public function fieldsFormDisplay($rid = 0)
    {
        global $_W, $_GPC;
        if (!empty($rid)) {
            $reply    = pdo_fetch("SELECT * FROM " . tablename('amouse_weijob_reply') . " WHERE rid = :rid", array(
                ':rid' => $rid
            ));
            $sql      = 'SELECT id,title,thumb,content FROM ' . tablename('amouse_weijob_company') . ' WHERE `id`=:companyid';
            $activity = pdo_fetch($sql, array(
    ...
    public function fieldsFormSubmit($rid)
    {
        ...
        if ($reply) {
            pdo_update('amouse_weijob_reply', $record, array(
                'id' => $reply['id']
            ));
        } else {
    ...
    {
        pdo_delete('amouse_weijob_reply', array(
            'rid' => $rid
        ));
    }
    public function settingsDisplay($settings)
    {
        ...
        if (checksubmit()) {
            ...
            $cfg['nickname']  = $_GPC['nickname'];
            $cfg['openid']    = $_GPC['openid'];
            $cfg['tpl']       = $_GPC['tpl'];
