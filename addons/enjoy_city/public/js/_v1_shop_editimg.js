...
initEvents.push(function(){$(".crop-row").on("click",function()
		{$("#img-dialog img").attr("src",$(this).attr("src"));mui("#img-dialog").popover("show")});
(function($){var serverCompress=0;function isCanvasSupported(){var t=document.createElement("canvas");return!!(t.getContext&&t.getContext("2d"))}function compress(t,a){var o=new FileReader;o.onload=function(t){var o=$("<img/>");o.on("load",function(){var t=800;var o=document.createElement("canvas");var e=o.getContext("2d");var i;var r;i=this.width;r=this.height;if(this.width>this.height){t=Math.min(t,this.height);i=Math.round(t*this.width/this.height);r=t}else{t=Math.min(t,this.width);r=Math.round(t*this.height/this.width);i=t}o.width=i;o.height=r;e.clearRect(0,0,o.width,o.height);e.drawImage(this,0,0,i,r);var n=o.toDataURL("image/jpeg");if(!serverCompress){a(n)}else{window.API.post("pic_compress",{upload:n},function(t){if(t.error==0){a(t.data)}else{showMessage("压缩失败，请重试")}})}});o.attr("src",t.target.result)};o.readAsDataURL(t)}$("#crop-cancel-btn").off("click").on("click",function(){mui("#crop-dialog").popover("hide")});$("#crop-submit-btn").off("click").on("click",function(){var url=$(this).data("url");var imgWidth=$(this).data("width");var imgHeight=$(this).data("height");var callback=$(this).data("callback");var field=$(this).data("field");var id=$(this).data("id");var options={};if(parseInt(imgWidth,10)>0){options.width=imgWidth}if(parseInt(imgHeight,10)>0){options.height=imgHeight}var imgData=window.cropObject.getCroppedCanvas(options).toDataURL("image/jpeg");var progressbar=$(".upload-progress");var upfield=$(this).data("upload");var xhr=new XMLHttpRequest;var upload=xhr.upload;var that=$(this);that.attr("disabled",true);upload.addEventListener("progress",function(t){if(t.lengthComputable){progressbar.css("width",t.loaded/t.total*100+"%")}},0);upload.addEventListener("load",function(t){},0);upload.addEventListener("error",function(t){alert("Error:"+t)},0);xhr.open("POST",url?url:DIR+"?c=api-upload");xhr.overrideMimeType("application/octet-stream");var formPost=new FormData;formPost.append("upload",imgData);formPost.append("type",upfield);formPost.append("field",field);formPost.append("id",id);formPost.append("base64",1);formPost.append("FORM_HASH",FORM_HASH);xhr.onreadystatechange=function(){if(4==xhr.readyState){switch(xhr.status){case 200:var data={};try{eval("data="+xhr.responseText);console.log(data)}catch(e){}if(data&&data.error==0){$("#"+upfield).attr("src",data.extend.url).show();mui("#crop-dialog").popover("hide");callback&&window[callback](data)}else{mui.toast(data.data)}break;default:mui.toast("上传失败，请重试");break}that.removeAttr("disabled")}};xhr.send(formPost)});$("#crop-plus-btn").off("click").on("click",function(){window.cropObject.zoom(.1)});$("#crop-dec-btn").off("click").on("click",function(){window.cropObject.zoom(-.1)});$("#crop-rotate-btn").off("click").on("click",function(){window.cropObject.rotate(90)});var fileElement=false;var getInputFile=function(t,a){if(!fileElement){try{fileElement=$('<input accept="image/*" type="file" style="opacity: 0;" /'+">");$(document.body).append(fileElement);fileElement.on("click",function(){fileElement.val("")}).on("change",a)}catch(o){alert(o.number+"-"+o.message+":"+o.descrption+"(创建INPUT=File失败)")}}fileElement.attr(t);return fileElement};window.cropObject=false;var cropOptions={};$.fn.mzCrop=function(){$(this).on(window.CLICK_EVENT,function(){var t=this;var a={accept:($(t).data("accept")||"image")+"/*",type:"file"};var o=parseInt($(t).data("clicks"),10);if(!o){o=0}o++;$(t).data("clicks",o);if(o>3){if(confirm("系统检测到您多次点击上传按钮，是否无法上传文件？")){alert(["解决方案1：","安装 QQ浏览器，安装好后，重启微信，再次上传。"].join("\r\n"));alert(["解决方案2：","使用UC、猎豹、搜狗、QQ等浏览器输入网址进行登记：",document.domain].join("\r\n"))}}cropOptions={};if($(t).data("maxwidth")){cropOptions.maxCropBoxWidth=$(t).data("maxwidth");$("#crop-submit-btn").data("width",cropOptions.maxCropBoxWidth)}if($(t).data("minwidth")){cropOptions.minCropBoxWidth=$(t).data("minwidth");$("#crop-submit-btn").data("width",cropOptions.minCropBoxWidth)}if($(t).data("maxheight")){cropOptions.maxCropBoxHeight=$(t).data("maxheight");$("#crop-submit-btn").data("height",cropOptions.maxCropBoxHeight)}if($(t).data("minheight")){cropOptions.minCropBoxHeight=$(t).data("minheight");$("#crop-submit-btn").data("height",cropOptions.minCropBoxHeight)}if(parseInt($(t).data("aspectratio"),10)>-1){cropOptions.aspectRatio=$(t).data("aspectratio")}if($(t).data("autocrop")){cropOptions.autoCropArea=$(t).data("autocrop")}if(parseInt($(t).data("viewmode"),10)>-1){cropOptions.viewMode=$(t).data("viewmode")}else{}$("#crop-submit-btn").data("upload",$(t).data("upload")).data("id",$(t).data("id")).data("field",$(t).data("field"));if(false&&parseInt(UID,10)==12){if(!WX_INIT){alert("微信接口未配置，无法上传")}wx.chooseImage({count:1,sizeType:["original","compressed"],sourceType:["album","camera"],success:function(t){if(t.errMsg!="chooseImage:ok"){alert("上传图片失败，请重试")}var a=t.localIds.pop();setTimeout(function(){wx.uploadImage({localId:a,isShowProgressTips:1,success:function(t){var a=t.serverId;console.log(t);mui("#crop-dialog").popover("show");if(window.cropObject){window.cropObject.destroy();window.cropObject=null}var o=$("<span>图片加载、压缩处理中...</span>");$("#crop-dialog .crop-img").html("").append(o).append('<img class="upload" />');var e=$("#crop-dialog img")[0];e.onload=function(){if(e.complete){cropOptions=$.extend({},{modal:true,minCropBoxWidth:5,minCropBoxHeight:5,crop:function(t){}},cropOptions);window.cropObject=new Cropper(e,cropOptions);o.remove()}};e.src=DIR+"?c=api-get_wx_media&id="+a+"&FORM_HASH="+FORM_HASH}})},0)}})}else{try{var e=getInputFile(a,function(t){t.stopPropagation();t.preventDefault();var o=t.dataTransfer!==undefined?t.dataTransfer.files[0]:t.target.files[0];if(!isCanvasSupported()){alert("您的浏览器不支持上传图片，请更换其它浏览器尝试");return}var e=new RegExp(a.accept,"ig");if(!o.type.search(e)==-1){return}mui("#crop-dialog").popover("show");if(window.cropObject){window.cropObject.destroy();window.cropObject=null}var i=$("<span>图片加载、压缩处理中...</span>");$("#crop-dialog .crop-img").html("").append(i).append('<img class="upload" />');var r=$("#crop-dialog img")[0];compress(o,function(t){r.src=t;cropOptions=$.extend({},{modal:true,minCropBoxWidth:5,minCropBoxHeight:5,crop:function(t){}},cropOptions);window.cropObject=new Cropper(r,cropOptions);i.remove()})});e[0].click()}catch(i){try{e.click()}catch(r){var n=i.number+"-"+i.message+":"+i.descrption+"\r\n"+r.number+"-"+r.message+":"+r.descrption;alert(n)}}}});return this}})(window.Zepto||window.jQuery);$("*[data-upload]").mzCrop()});;
