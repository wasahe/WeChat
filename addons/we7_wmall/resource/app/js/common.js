			...
			'<div class="list-block media-list">\
				<ul>\
					...
					<li>\
						...
						<input type="radio" name="light7-select" id="weui-select-id-{{this.title}}" value="{{this.value}}" {{#if this.checked}}checked="checked"{{/if}} data-title="{{this.title}}">\
			...
			'<div class="list-block media-list">\
					<ul>\
						...
						<li>\
							...
							<input type="checkbox" name="light7-select" id="weui-select-id-{{this.title}}" value="{{this.value}}" {{#if this.checked}}checked="checked"{{/if}} data-title="{{this.title}}">\
								<div class="item-media"><i class="icon icon-form-checkbox"></i></div>\
							</label>\
						</li>\
						{{/items}}\
...
$(function(){
	$(document).on("click", "a", function(e) {
		...
		if(!url || url === "#" || /javascript:.*;/.test(url) || /tel:.*/.test(url)) return;
		$.showIndicator();
	})
});