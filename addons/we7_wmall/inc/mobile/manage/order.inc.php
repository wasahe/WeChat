...
if($op == 'status') {
	...
	if($status == 7) {
		...
		if($status == '3' && $order['deliveryer_id'] > 0) {
			message(error(-1, "该订单已被配送员接单,不能重复通知配置员"), '', 'ajax');
		}
...
if($op == 'consume') {
	...
	if($order['status'] == 5) {
		$this->imessage('该订单已核销，请勿重复核销', $this->createMobileUrl('mgorder', array('op' => 'list')), 'error');
	}
