...
if($op=='list'){
	...
	$lists = pdo_fetchall("SELECT * FROM ".tablename($this->shake_user_table)." WHERE {$where} ORDER BY count DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,array(':weid'=>$weid,':rid'=>$rid,':rotate_id'=>$rotate_id));
...
if(checksubmit('downall')){
			$up_list = pdo_fetchall("SELECT * FROM ".tablename($this->shake_user_table)." WHERE  weid=:weid  AND rid=:rid AND rotate_id=:rotate_id ORDER BY count DESC ", array(':weid'=>$weid,':rid'=>$rid,':rotate_id'=>$rotate_id));
