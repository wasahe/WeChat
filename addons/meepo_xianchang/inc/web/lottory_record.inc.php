...
if($op == 'list') {
	...
	$psize = 20;
	$sql = "SELECT * FROM ".tablename($this->lottory_user_table)." WHERE  weid=:weid AND rid=:rid ORDER BY lottory_id DESC,createtime ASC LIMIT ".($pindex - 1) * $psize.",{$psize}";
	...
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->lottory_user_table) . " WHERE  rid=:rid AND weid=:weid", array(':rid' =>$rid,':weid'=>$weid));
