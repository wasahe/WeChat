...
if($_W['isajax']){
	...
	if($last_id==0){
		$wall = pdo_fetchall("SELECT * FROM ".tablename($this->wall_table)." WHERE weid = :weid AND rid = :rid AND status = :status ORDER BY createtime DESC LIMIT 0,10",array(':weid'=>$weid,':rid'=>$rid,':status'=>'1'));
		$data = array();
	}else{
