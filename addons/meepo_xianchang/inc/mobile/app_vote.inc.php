...
if($vote_id){
	...
	$vote_xms = pdo_fetchall("SELECT * FROM ".tablename($this->vote_xms_table)." WHERE rid=:rid AND weid=:weid AND fid=:fid ORDER BY displayid   ASC",array(':rid'=>$rid,':weid'=>$weid,':fid'=>$vote_id));
}else{
	...
	$vote_id  = $vote['id'];
	$vote_xms = pdo_fetchall("SELECT * FROM ".tablename($this->vote_xms_table)." WHERE rid=:rid AND weid=:weid AND fid=:fid ORDER BY displayid    ASC",array(':rid'=>$rid,':weid'=>$weid,':fid'=>$vote['id']));
}
if(empty($vote)){
