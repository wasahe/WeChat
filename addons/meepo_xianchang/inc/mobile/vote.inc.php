...
if($vote_id){
	...
	$vote_xms = pdo_fetchall("SELECT * FROM ".tablename($this->vote_xms_table)." WHERE rid=:rid AND weid=:weid AND fid=:fid ORDER BY displayid ASC ,nums ASC",array(':rid'=>$rid,':weid'=>$weid,':fid'=>$vote_id));
}else{
	$vote = pdo_fetch("SELECT * FROM ".tablename($this->vote_table)." WHERE rid=:rid AND weid=:weid ORDER BY id ASC",array(':rid'=>$rid,':weid'=>$weid));
	$vote_id  = $vote['id'];
	$vote_xms = pdo_fetchall("SELECT * FROM ".tablename($this->vote_xms_table)." WHERE rid=:rid AND weid=:weid AND fid=:fid ORDER BY displayid ASC ,nums ASC",array(':rid'=>$rid,':weid'=>$weid,':fid'=>$vote['id']));
}
