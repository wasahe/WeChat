...
if($_W['isajax']){
	...
	$psize = 6;
	$news = pdo_fetchall("SELECT * FROM ".tablename($this->wall_table)." WHERE openid=:openid AND weid=:weid  ORDER BY createtime DESC LIMIT " . $pindex . ",{$psize}",array(':openid'=>$openid,':weid'=>$weid));
