...
if($_W['isajax']){
	...
	pdo_insert($this->lottory_user_table ,$insert);
	...
	if($lottory_config['send_mess']>0){
		$award = pdo_fetch("SELECT `tag_name`,`luck_name` FROM ".tablename($this->lottory_award_table)." WHERE weid=:weid AND rid=:rid AND id=:id",array(':weid'=>$weid,':rid'=>$rid,':id'=>$award_id));
