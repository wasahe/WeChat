...
if ($operation == 'display') {
    ...
    ca('virtual.data.add|virtual.data.edit');
    $typeid = $_GPC['typeid'];
    $editid = $_GPC['id'];
    if (empty($typeid)) {
        message("Url参数错误！请重试！", $this->createPluginWebUrl('virtual/temp'), 'error');
    ...
    if ($_W['ispost']) {
        ...
        if (!empty($typeid)) {
            ...
            if (!empty($item['fields'])) {
                ...
                plog('virtual.data.edit', "修改数据 模板ID: {$typeid}");
                if (!empty($noinsert)) {
                    $tip = '<br>未保存成功的数据：主键=' . $noinsert . '<br>失败原因：已经使用无法更改';
                    message('部分数据保存成功！' . $tip, '', 'warning');
                } else {
                    message('保存成功！', $this->createPluginWebUrl('virtual/data', array(
                        'typeid' => $typeid
                    )));
                }
            }
    ...
    if (!empty($id)) {
        ...
        if (!empty($type['openid'])) {
            message("数据已使用，无法删除！", $this->createPluginWebUrl('virtual/data', array(
                'typeid' => $typeid
            )), 'error');
        }
        ...
        $this->model->updateStock($typeid);
        plog('virtual.data.delete', "删除数据 模板ID: {$typeid} ID: {$id}");
        message('删除成功！', $this->createPluginWebUrl('virtual/data', array(
            'typeid' => $typeid
        )));
    } else {
        message('Url参数错误！请重试！', $this->createPluginWebUrl('virtual/data', array(
            'typeid' => $typeid
        )), 'error');
    }
    exit;
