...
$list = pdo_fetchall('SELECT log.*, m.avatar,m.nickname,m.realname,m.mobile,m1.avatar as avatar1,m1.nickname as nickname1,m1.realname as realname1,m1.mobile as mobile1 FROM ' . tablename('sz_yi_poster_log') . ' log ' . ' left join ' . tablename('sz_yi_member') . ' m1 on m1.openid = log.openid and m1.uniacid = log.uniacid ' . ' left join ' . tablename('sz_yi_member') . ' m on m.openid = log.from_openid  and m.uniacid = log.uniacid' . " WHERE 1 {$condition} ORDER BY log.createtime desc " . '  LIMIT ' . ($pindex - 1) * $psize . ',' . $psize, $params);
