...
//decode by QQ:3213288469 http://www.5kym.com/
...
class Weixin_authModuleSite extends WeModuleSite
{
	...
	public function doWebMpAdd()
	{
		...
		if (isset($_GPC['type']) && $_GPC['type'] == 'add') {
			...
			$id = pdo_insertid();
			if ($res) {
				$this->JSON(array(200, '添加成功', array('name' => $data['name'], 'url' => $this->getJoinUrl() . "&d={$id}", 'token' => $data['token'], 'key' => $data['encodingaeskey'], 'checkUrl' => $this->createWebUrl('MpCheck', array('id' => $id)), 'appListUrl' => $this->createWebUrl('AppList', array('id' => $id)))));
			} else {
				$this->JSON(array(203, '添加失败'));
			}
		}
	...
	public function doWebMpEdit()
	{
		...
		if (isset($_GPC['type']) && $_GPC['type'] == 'edit') {
			if (!$id) {
				$this->JSON(array(201, '公众号不存在'));
			}
			if (empty($_GPC['name'])) {
				$this->JSON(array(203, '名称不能为空'));
			}
			$data = array('name' => htmlspecialchars($_GPC['name']), 'desc' => htmlspecialchars($_GPC['desc']));
			if (pdo_update('mp_list', $data, array('id' => $id, 'w_id' => $_W['uniacid']))) {
				$this->JSON(array(200, '修改成功'));
			} else {
				$this->JSON(array(205, '修改失败'));
			}
		}
		if (!$id) {
			message('公众号不存在!', $this->createWebUrl('MpList'), 'error');
		}
	...
	public function doWebMpDel()
	{
		...
		if (pdo_update('mp_list', array('is_delete' => 1), array('id' => $id, 'w_id' => $_W['uniacid']))) {
			$this->JSON(array(200, '删除成功'));
		} else {
			$this->JSON(array(203, '删除失败'));
		}
	}
	public function doWebMpDisable()
	{
		...
		if (pdo_query($sql)) {
			$this->JSON(array(200, '操作成功'));
		} else {
			$this->JSON(array(203, '操作失败'));
		}
	}
	...
	public function doWebMpCheck()
	{
		...
		$info = $this->getMpInfo($id);
		if (!$info) {
			$this->JSON(array(203, '公众号不存在'));
		}
		if ($info['is_yz'] == 1) {
			$this->JSON(array(200, '接入成功'));
		} else {
			$this->JSON(array(205, '未接入'));
		}
	}
	...
	public function doWebAppAdd()
	{
		...
		if (isset($_GPC['type']) && $_GPC['type'] == 'add') {
			if (empty($_GPC['name'])) {
				$this->JSON(array(201, '请填写平台名称'));
			}
			if (empty($_GPC['url'])) {
				$this->JSON(array(203, '请填写Url'));
			}
			if (empty($_GPC['tokens'])) {
				$this->JSON(array(205, '请填写Token'));
			}
	...
	public function doWebAppEdit()
	{
		...
		if (!$appid) {
			message('平台不存在!', $this->createWebUrl('AppList', array('id' => $info['id'])), 'error');
		}
	...
	public function doWebAppDel()
	{
		...
		if (!$appid) {
			$this->JSON(array(203, '平台不存在'));
		}
		if (pdo_update('mp_app', array('is_delete' => 1), array('id' => $appid, 'm_id' => $id))) {
			$this->JSON(array(200, '操作成功'));
		} else {
			$this->JSON(array(205, '操作失败'));
		}
	}
	...
	public function doWebStatic()
	{
		...
		if (isset($_GPC['mp'])) {
			...
			for ($i = 6; $i >= 0; $i--) {
				...
				if ($Mpids) {
					$value = pdo_fetchall("SELECT count(id) as value,a_id FROM " . tablename('mp_log') . " WHERE m_id = :mid AND time between " . strtotime(date('Y-') . $key) . ' AND ' . (strtotime(date('Y-') . $key) + 86399 . ' AND a_id in (' . implode(',', $Mpids) . ') GROUP BY a_id'), array(':mid' => $id));
				}
				for ($k = 0, $n = count($MpLists); $k < $n; $k++) {
					if ($MpLists[$value[$k]['a_id']]) {
						$MpLists[$value[$k]['a_id']]['data'][$i] = $value[$k]['value'] + 0;
					}
					if (!isset($MpLists[$MpListKeys[$k]]['data'][$i])) {
						$MpLists[$MpListKeys[$k]]['data'][$i] = 0;
					}
				}
				$days[] = $key;
			}
